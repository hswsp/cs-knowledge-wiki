...
4. **逆元**：对任意 a 属于 G，存在 a<sup>(-1) </sup>属于 G，使得 a * a<sup>(-1)</sup> = a<sup>(-1)</sup> * a = e。
...
另一种方法是费马小定理：$a^{-1} = a^{p-2} \bmod p$。这在 p 很大时可以用快速幂高效计算，但在 p 较小时不如扩展欧几里得直接。
...
$$
a \times b = g^{(\log_g(a) + \log_g(b)) \bmod 255}
$$
...
$$
D(x) = d_0 x^{n-1} + d_1 x^{n-2} + \cdots + d_{k-1} x^{n-k}
$$
...
### 原理
1. 选择一个 k-1 次随机多项式 $f(x) = s + a_1 x + a_2 x^2 + \dots + a_{k-1} x^{k-1}$，其中常数项是秘密 s。
