...
$$
这其实就是在说：先把 $u$ 和 $v$ **看作列空间**里的**坐标**，然后通过 $A$ 映射到 $\mathbb{R}^m$里（向量），再计算标准点积。
...
用$A$ 的行空间内的向量表示$A$：
$$
A = \begin{bmatrix} v_1^T \\ v_2^T \\ \vdots \\ v_m^T \end{bmatrix}
$$

$$
A^TA = [v_1, v_2, \dots, v_m] \begin{bmatrix} v_1^T \\ v_2^T \\ \vdots \\ v_m^T \end{bmatrix} = v_1 v_1^T + v_2 v_2^T + \dots + v_m v_m^T
$$

这就正是每个行向量的外积！那么对于 $\forall x \in C(A^T)$,  $A^TA x$ 的意思**向$A$的每个行向量做投影！**。

我们需要找的是行空间的标准正交基！也就是此时$A^TA$是正交投影矩阵。

$A^T A$ 一定是半正定的(PSD)。那么对任意对称半正定矩阵 **$A^T A$ 的特征值 $\lambda$** 与对应的单位特征向量 $v$：
$$
\boxed{A^T A v = \lambda v}
$$
