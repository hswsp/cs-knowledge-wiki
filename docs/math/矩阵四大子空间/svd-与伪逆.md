...
把 $\mathbb{R}^m$ 沿正交直和拆开：$\mathbb{R}^m = C(A) \oplus N(A^T)$。任意 $b$ 唯一分解为

$$b = b_{\parallel} + b_{\perp},\quad b_{\parallel}\in C(A),\ b_{\perp}\in N(A^T),\ b_{\perp}\perp C(A).$$
...
对 $AA^+$：由 P1 得 $(AA^+)^2 = AA^+ A A^+ = AA^+$（幂等），由 **P3** 得对称——所以它就是一个正交投影矩阵；又 $\mathrm{Im}(AA^+) = C(A)$，故是投到 $C(A)$ 上。$A^+A$ 同理（用 P2、P4），投到 $C(A^T)$。
...
$$
V_r V_r^T = [v_1, v_2, \dots, v_r] \begin{bmatrix} v_1^T \\ v_2^T \\ \vdots \\ v_r^T \end{bmatrix} = v_1 v_1^T + v_2 v_2^T + \dots + v_r v_r^T
$$
...
**行空间（Row Space）就是由这 $r$ 条直线张成的（就像用 $r$ 根柱子撑起一个房间）。**

所以，把所有这些微型投影仪加起来：

$$
P_{\text{Row}(A)} = v_1 v_1^T + v_2 v_2^T + \dots + v_r v_r^T = \sum_{i=1}^r v_i v_i^T
$$
