...
* 核对每个配置项的作用和影响，识别出关键配置项。进行多种场景的性能测试。
* 进行压力测试，连续跑几天，观察 CPU、内存、磁盘 I/O 等指标波动。
* 进行故障测试：kill、断电、拔网线、重启 100 次以上、切换等。

### 小心应用，灰度发布

如我们做了上面的“深入研究、仔细测试”，发现没什么问题，是否就可以放心大胆地应用到线上了呢？别高兴太早，即使你的研究再深入，测试再仔细，还是要小心为妙，因为再怎么深入地研究，再怎么仔细地测试，都只能降低风险，但不可能完全覆盖所有线上场景。
