...
- 向量数据库（Milvus、Qdrant 等）、对象存储（Ozone、MinIO 的元数据层等）需要**选主做元数据/索引分区**，需要一个稳定、可理解、可运维的一致性协议。
...
| **MinIO** | Go | 不用 Raft（纠删码 erasure code） | 对象存储 | ❌ 反例：选主用更轻的机制，数据靠 EC |
...
- **MinIO / MongoDB 作为反例**很有价值：MinIO 用**纠删码（EC）**而非复制日志来保证数据可用，元数据用轻量选主；MongoDB 用自研的 primary-选举机制。这说明"Raft 不是唯一答案"，也帮我们理解 Raft 到底在解决什么、什么时候该用它。
