...
`impl<St: ?Sized + Unpin> Unpin for Next<'_, St> {}`。这里是为Next类型实现Unpin trait。St有两个约束：`?Sized`表示允许不定大小类型，Unpin表示类型可以安全移动。`Next<'_, St>`使用匿名生命周期，说明它的生命周期与结构体自身一致。自动实现Unpin是因为Next不包含任何需要固定地址的字段，依赖St的Unpin。
`impl<'a, St: ?Sized + Stream + Unpin> Next<'a, St> { ... }`。这里定义了`Next`的构造函数。`'a`是显式生命周期参数，St需要实现`Stream`和`Unpin`。`pub(super) fn new`表示这个构造函数只在父模块中可用，接收`&'a mut St`参数，确保Next实例的生命周期不超过流对象。
...
用户提供的代码中，初始状态是`(n, (1, 1))`。这里的n应该是一个外部变量，表示要生成斐波那契数列的项数。而(1, 1)可能是斐波那契数列的前两个初始值，但通常斐波那契数列以0和1或1和1开始，所以这里可能需要确认。
接下来，闭包的参数是`|(mut n, (a, b))|`，这说明初始状态被解构为两个部分：`mut n`和元组`(a, b)`。每次迭代，闭包会生成下一个元素并更新状态。在代码中，每次生成c = a + b，然后更新状态为(n, (b, c))，这样下一次迭代的a和b就变成了之前的b和c，从而生成下一个斐波那契数。
- **自引用结构**：
当 `unfold` 的闭包捕获了环境变量（如你的例子中的 `n, a, b`）时，生成的 `Stream` 会包含对这些变量的引用。如果这些变量在 `async` 块中被修改，就会形成自引用结构
- **Future 的移动问题**：
`async move` 会获取变量的所有权，但 `unfold` 需要保持状态在多次 `poll` 之间持续存在，导致 Future 必须携带自己的状态，形成自引用
- **Unpin 的要求**：
标准库中大部分组合器需要 `Stream: Unpin`， 自引用结构无法安全实现 `Unpin`（移动会导致指针失效），因此编译器自动推导出 `impl !Unpin for GeneratedStream`
**内存布局示意图**：
解决方法：
- 使用 `Box::pin` 固定到堆内存：
- 在 async 块中避免捕获可变变量：
- 使用 `pin-project` 等工具安全处理自引用：
这个问题的本质是 Rust 的所有权系统与异步状态机交互时产生的约束，理解自引用和 `Pin` 的关系是解决这类问题的关键。
值得注意的是，**使用 **`**unfold**`** 的时候，同时使用了局部变量和 **`**Future**`**，所以生成的 Stream 没有实现 Unpin**，我们在使用的时候，需要将其 pin 住。怎么做呢？
`Pin<>` 是一种很简单的方法，能将数据 Pin 在堆上，我们可以使用 StreamExt 的 boxed() 方法来生成一个 `Pin<Box<>>`。
...
注意，由于 `poll_next_line()` 的结果是 `Result<>`，而 Stream 的 `poll_next()` 的结果是 `Option<>`，所以我们需要使用 Result 方法的 [transpose](https://doc.rust-lang.org/std/result/enum.Result.html#method.transpose) 来将二者对调。这个`transpose` 方法是一个很基础的方法，非常实用。
# 异步 IO 接口
...
其中三个方法和 Item 是无关的，这会导致，如果不同的输入类型有多个实现，Sink的`poll_ready`、`poll_flush` 和 `poll_close` 可能会有重复的代码。所以一般我们在使用 Sink 时，如果确实需要处理不同的数据类型，可以用 enum 将它们统一（感兴趣的话，可以进一步阅读这个[讨论](https://github.com/rust-lang/futures-rs/issues/623)）。
