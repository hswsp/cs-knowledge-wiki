...
## 内存管理
我们在[01 | 内存：值放堆上还是放栈上，这是一个问题](https://www.yuque.com/yangguangfanxing/vd7id1/mvctrvemvlmmo36b) 说过堆和栈，它们是代码中使用内存的主要场合。
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/c7eec71df0fdc85c905c60ab78db15d7.jpg)
...
我们看下图的对比总结：
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/e381fa9ab73036480df9c8a182dab4b1.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/0a1151168af2a19a23f8db16df101cc4.jpg)
为什么 Rust 编译器会这么做呢？
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/f724ce693e12745d7ba976044e5aa937.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/3db1a2ccebf936aa10d9c1bd7fcf8a42.jpg)
...
根据刚才说的三条对齐规则，tag 后的内存，会根据其对齐大小进行对齐，所以对于 Option，其长度是 1 + 1 = 2 字节，而 `Result`，长度是 8 + 8 =16 字节。一般而言，64 位 CPU 下，enum 的最大长度是：最大类型的长度 + 8，因为 64 位 CPU 的最大对齐是 64bit，也就是 8 个字节。
下图展示了 enum、Option以及Result 的布局：
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/f22b2700de556385efbc44f04dd6b982.jpg)
...
而 [Vec结构](https://doc.rust-lang.org/src/alloc/vec/mod.rs.html#294-302)是 3 个 word 的胖指针，包含：一个指向堆内存的指针pointer、分配的堆内存的容量capacity，以及数据在堆内存的长度length，如下图所示：
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/c2234ab79c2eaf2c76698c8f11b24d37.jpg)
很多动态大小的数据结构，在创建时都有类似的内存布局：**栈内存放的胖指针，指向堆内存分配出来的数据**，我们之前介绍的 Rc 也是如此。
关于值在创建时的内存布局，今天就先讲这么多。如果你对其它数据结构的内存布局感兴趣，可以访问 [cheats.rs](https://cheats.rs/#data-layout)，它是 Rust 语言的备忘清单，非常适合随时翻阅。比如，引用类型的内存布局：
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/98e80265d8fbe77a9c6e53df22efafa5.png)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/4a0fd813e6722c9efef3c8be91b3d155.jpg)
...
不过也有一个例外，要说明：对栈上的大数组传参，由于需要复制整个数组，会影响效率。所以，一般我们**建议在栈上不要放大数组（比如**`**let arr_with_default = [0; 10];**`**）**，如果实在需要，那么传递这个数组时，最好用传引用而不是传值。
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/0c0d83776e12a099453c837c997d93ae.jpg)
如果要释放的值是一个复杂的数据结构，比如一个结构体，那么这个结构体在调用 `drop()` 时，**会依次调用每一个域的 **`**drop()**`** 函数**，如果域又是一个复杂的结构或者集合类型，就会递归下去，直到每一个域都释放干净。
我们可以看这个例子：
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/b80ec8eac77944512626bdfc3byy0094.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/7d3760f88297fc5900c74193cae5e39b.jpg)
...
Rust 的枚举需要能够存储所有变体，因此 `Result<String, ()>` 的大小取决于：
- `**Ok(String)**`** 的大小**：24 字节（`String` 的大小）。
- `**Err(())**`** 的大小**：0 字节（`()` 的大小）。
为了区分 `Ok` 和 `Err`，Rust 需要一个额外的 **标签字节**（discriminant tag），通常为 1 字节。
因此，在 64 位系统上，`Result<String, ()>` 的栈上大小为： **24 字节（**`**String**`**） + 1 字节（标签） = 24 字节（对齐优化后依然是 24 字节）**
