...
这也是 Rust 处理很多问题的思路：编译时，处理大部分使用场景，保证安全性和效率；运行时，处理无法在编译时处理的场景，会牺牲一部分效率，提高灵活性。后续讲到**静态分发**和**动态分发**也会有体现，这个思路很值得我们借鉴。
那具体如何在运行时做**动态检查**呢？运行时的动态检查又如何与编译时的静态检查自洽呢？
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/a3510f9b565577bc74bc0dcda0b3e78c.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/9f1a17dea75f9cae596a56f51d007ccd.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/0c5b0ff12963792a55baa43d3b3054ab.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/94bd27a93210ea829482663c9138de3c.jpg)
### 实现可修改DAG
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/6264d51da5c5e9025abf28d7c0dd2e46.jpg)
...
然而，Rc/Arc 是**不可变**的，如果想要修改内部的数据，**需要引入内部可变性**，在单线程环境下，可以在 Rc 内部使用 `RefCell`；在多线程环境下，可以使用 Arc 嵌套 Mutex 或者 RwLock 的方法。
你可以看这张表快速回顾：
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/fc524d667fabeec0a8a22d0e10531086.jpg)
## 思考题
- 运行下面的代码，查看错误，并阅读 [std::thread::spawn](https://doc.rust-lang.org/std/thread/fn.spawn.html) 的文档，找到问题的原因后，修改代码使其编译通过。
这段代码的问题在于，闭包中的线程试图访问主线程中的 `arr`，但是 `arr` 的所有权并未被移动到新线程中。Rust 的编译器会报错，因为新线程可能在 `arr` 被销毁后仍然访问它，从而违反 Rust 的所有权和线程安全规则。
解决方案是将 `arr` 的所有权转移到新线程中，或者使用线程安全的引用计数类型（如 `Arc`）。
以下是两种解决方案：
**方案 1：移动所有权**
将 `arr` 的所有权转移到新线程中，使用 `move` 关键字：
说明：
- `move` 将 `arr` 的所有权移动到闭包中，使得新线程独占 `arr`。
- 使用 `.join()` 确保主线程等待新线程完成。
**方案 2：使用 **`Arc`** 实现共享所有权**
如果需要多个线程共享数据，可以使用 `Arc`（原子引用计数）：
说明：
- `Arc` 提供线程安全的共享所有权。
- `Arc::clone(&arr)` 增加引用计数，而不是复制数据。
**如何选择？**
- 如果数据只需一个线程使用，使用 `move`。
- 如果数据需要多个线程共享，使用 `Arc`（或 `Arc<Mutex<T>>` 进行修改操作）。
**代码对比**
...
多线程共享数据（只读）
`Arc`
`let data = Arc::new(vec![1]); thread::spawn(move

多线程共享数据（读写）
`Arc<Mutex>` 或 `Arc<RwLock>`
`let data = Arc::new(Mutex::new(vec![1])); thread::spawn(move
确保根据数据访问的需求，选择合适的工具以满足 Rust 的线程安全保证。
- 你可以写一段代码，在 main() 函数里生成一个字符串，然后通过 `std::thread::spawn` 创建一个线程，让 main() 函数所在的主线程和新的线程共享这个字符串么？提示：使用 [std::sync::Arc](https://doc.rust-lang.org/std/sync/struct.Arc.html)。
...
- `self.inner()` 应该返回一个类型，比如 `&Cell<T>`、`&RefCell<T>`，或 `&UnsafeCell<T>`。
- 调用 `inc_strong()` 时，`self.inner()` 内部通过这些类型提供了 **内部可变性** 的能力。
