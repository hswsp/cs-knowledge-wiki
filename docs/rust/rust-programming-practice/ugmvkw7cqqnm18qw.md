...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/7185e631b5700f85e8b0e4dc5d0aedf8.jpg)
...
- **对于同一个类型（struct）的同一个 trait 行为，可以有不同的实现**，比如我们之前大量使用的 From，此时可以用泛型 trait。
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/4900097edab0yye11233e14ef857be1d.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/9ddeafee9740e891f6bf9c1584e6905e.jpg)
...
![图片](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/%e9%99%88%e5%a4%a9%20%c2%b7%20Rust%20%e7%bc%96%e7%a8%8b%e7%ac%ac%e4%b8%80%e8%af%be/assets/59bd1c6f90b99e9604e6602e33a622e6.jpg)
...
- 如下代码能编译通过么，为什么？
这段代码无法编译的原因在于 `by_ref` 方法的使用。具体来说，`by_ref` 方法对于 `&mut dyn Write` 类型的引用不可用，因为它会产生生命周期和借用规则的冲突。
**问题分析**
在 Rust 中，`by_ref` 方法用于返回对象的可变引用。问题出现在你使用了 `by_ref` 对 `&mut dyn Write` 类型的引用进行操作。
`by_ref` 方法返回的引用是对原对象的一个可变引用。但是，`w` 是一个 `&mut dyn Write`，它已经是一个可变引用，因此，调用 `by_ref()` 会导致生命周期冲突，因为 `w1` 和 `w` 会共享对同一个对象的可变引用，而在 Rust 中，不允许同时存在多个可变引用。
**问题的根源**
在 Rust 中，**同时拥有多个可变引用是不允许的**。这是为了防止数据竞争和不可预测的行为。而 `by_ref` 方法本质上是返回一个新的可变引用，因此会违反这一规则。
#### 错误信息：
编译器通常会提示如下错误：
**修复方法**
要解决这个问题，有几个方法可以尝试：
**1. 避免使用 **`by_ref`
如果你只是想写入数据到文件，可以直接使用原来的可变引用 `w`，而不需要 `by_ref`：
这种方式会更加简洁，并且避免了生命周期和借用问题。
**2. 使用 **`std::io::BufWriter`**（可选）**
如果你确实需要使用 `by_ref` 来避免消费掉原始对象，可以考虑使用 `BufWriter` 等包装器，这些类型实现了类似 `by_ref` 的行为并允许对写入进行更多的控制。
例如，使用 `BufWriter`：
**总结**
- `by_ref` 不能直接用于 `&mut dyn Write`，因为它会导致对同一个对象的多个可变引用，从而违反 Rust 的借用规则。
- **解决方法**：直接使用 `w`，或者使用其他工具，如 `BufWriter`，来解决该问题。
- 在 Complex 的例子中，c1 + c2 会导致所有权移动，所以我们使用了 `&c1 + &c2` 来避免这种行为。除此之外，你还有什么方法能够让 c1 + c2 执行完之后还能继续使用么？如何修改 Complex 的代码来实现这个功能呢？
- 学有余力的同学可以挑战一下，[Iterator](https://doc.rust-lang.org/std/iter/trait.Iterator.html) 是 Rust 下的迭代器的 trait，你可以阅读 Iterator 的文档获得更多的信息。它有一个关联类型 Item 和一个方法 next() 需要实现，每次调用 next，如果迭代器中还能得到一个值，则返回 Some(Item)，否则返回 None。请阅读[如下代码](https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=6310cafc64afbd4762dd7997533f7d31)，想想看如何实现 SentenceIter 这个结构的迭代器？
