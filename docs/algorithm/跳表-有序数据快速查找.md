...
![](https://images.spumn.eu.cc/blog/112da0d3f88f140f.jpg)
...
![](https://images.spumn.eu.cc/blog/3f00a35156b4810f.jpg)
...
![](https://images.spumn.eu.cc/blog/890810fb387b7ade.jpg)
...
如果一个链表有 n 个结点，如果每两个结点抽取出一个结点建立索引的话，那么第一级索引的结点数大约就是 n/2，第二级索引的结点数大约为 n/4，以此类推第 m 级索引的节点数大约为 ![image](https://images.spumn.eu.cc/blog/8816d98ef32cf616.svg)。



假如一共有 m 级索引，第 m 级的结点数为两个，通过上边我们找到的规律，那么得出![image](https://images.spumn.eu.cc/blog/c7f09bd0a871f7b7.svg)，从而求得 ![image](https://images.spumn.eu.cc/blog/d7a7374fc8539f93.svg)。如果加上原始链表，那么整个跳表的高度就是![image](https://images.spumn.eu.cc/blog/abb4e42108028120.svg)。我们在查询跳表的时候，如果每一层都需要遍历 k  个结点，那么最终的时间复杂度就为 ![image](https://images.spumn.eu.cc/blog/41671b13e1c660ae.svg)。
...
![](https://images.spumn.eu.cc/blog/dde01cd3b1daa619.jpg)
...
跳表的空间复杂度分析并不难，如果一个链表有 n 个结点，如果每两个结点抽取出一个结点建立索引的话，那么第一级索引的结点数大约就是 n/2，第二级索引的结点数大约为 n/4，以此类推第 m 级索引的节点数大约为 ![image](https://images.spumn.eu.cc/blog/851d033173bd6e06.svg)，我们可以看出来这是一个等比数列。



这几级索引的结点总和就是 ![image](https://images.spumn.eu.cc/blog/e89b5a31fcb32fc8.svg)，所以跳表的空间复杂度为 o(n)。
...
![](https://images.spumn.eu.cc/blog/bfe31117432b2a97.jpg)
...
> 
>
> `Random.nextDouble() `uses `Random.next() `twice to generate a double that has approximately uniformly distributed bits in its mantissa, so it is uniformly distributed in the range 0 to ![image](https://images.spumn.eu.cc/blog/702420202e742a83.svg).
>
> 
>
...
> 
>
> Prior to scaling by 6, the output of `Math.random() `is one of ![image](https://images.spumn.eu.cc/blog/5051c8d3d05f3d77.svg)possible values drawn from a uniform distribution.
>
> 
>
> Scaling by 6 doesn't alter the number of possible values, and casting to an int then forces these values into one of six 'buckets' (0, 1, 2, 3, 4, 5), each bucket corresponding to ranges encompassing either 1501199875790165 (![image](https://images.spumn.eu.cc/blog/031a3a0d821e0c67.svg))or 1501199875790166 of the possible values (as 6 is not a disvisor of ![image](https://images.spumn.eu.cc/blog/5051c8d3d05f3d77.svg)). This means that for a sufficient number of dice rolls (or a die with a sufficiently large number of sides), the die will show itself to be **biased towards the larger buckets.**
>
> 
>
