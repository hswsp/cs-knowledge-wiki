---
title: "拓扑排序与名流问题"
description: "数据结构相关的算法无非两点：遍历 + 访问。那么图的基本遍历方法也很简单，以前就讲过如何从多叉树的遍历框架扩展到图的遍历。图这种数据结构还有一些比较特殊的算法，比如二分图判断，有环图无环图的判断，拓扑排序，以及最经典的最小生成树，单源最短路径问题，更难的就是类似网络流这样的问题。不过以我的经验..."
---
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHlRBUeeYfibj6KXSDSl93YNlaAg2ibCpibBbCIdtP4USfwT63O00ApDY8PIaia6REibMaOJ2tjprDTib5g/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHlRBUeeYfibj6KXSDSl93YNz3wWI3oWWb91IjunenedKZEeMsrfkBO4zkMR2yjYgIFqBIrlljOuvA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
public:
    bool canFinish(int numCourses, vector>& prerequisites) {
        ...
        for (int i = 0; i  > buildGraph(int numCourses, vector> &prerequisites) {
    // 图中共有 numCourses 个节点
...
void traverse(vector >&  graph, int s) {
    if (visited[s]) {
        // 出现环
        hasCycle = true;
    }
...
```cpp
for (int i = 0; i 我们需要`visited[s]`筛选掉之前的循环已经遍历的节点！！！否则会有大量重复遍历！！所以这里只有`visited[s] = true;`,最终会把所有节点都打上`true`。
​
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHlRBUeeYfibj6KXSDSl93YNcdAHa5s5PfznHKG3EbvAUAV6UkXa0vXlQ6RSpu7zttoFgFLI0bWX0g/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHlRBUeeYfibj6KXSDSl93YN0UuBgyqGfmNfeRxgaaeTwsib0yv2gN8ypiaMicusBnKmlcjyKycKHNjDQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHlRBUeeYfibj6KXSDSl93YNBPicG7rnptgfHyF2Lak1vV9CUj0W5Z9iau0bgsqa1jqhJfYiaI2apWXuw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
public:
    ...
    void travserse(vector> graph, int s){
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHlRBUeeYfibj6KXSDSl93YNoGcO1rcrpz1bxqfiay9puGiaOH0ibemS5KXzfNMBbFKqtwN6wBfwibt5yg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGkNwadoLRXGEaZLDKnzCOxseld4G93rblYibKfzoicfCsTK36N2eVGc59vFIdmz6cyxM0s9iaLH768w/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
class Solution {
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        if (numCourses [] adj = new HashSet[numCourses];
        for (int i = 0; i ();
        }
        ...
        for (int i = 0; i  successors = adj[head];
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdFibBVsIvI40151ibBRnIibfVn7ibHArVY2ib3rSjkz7wuk2AvCiciaMFMMjf23h5NlzQu41YNbd6XJDtViaQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdFibBVsIvI40151ibBRnIibfVnkEMUVNjSX3YYNybDZaE0VbEHUuPFnoYC0nsqjTYnwCSzEP3HLqWarQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdFibBVsIvI40151ibBRnIibfVn5KajqdQI8ctYTgh2Nqn7HAhiaEZTxvrvGmxssDPzYTYomPOdibYfiahlQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
