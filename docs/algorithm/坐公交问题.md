---
title: "坐公交问题"
description: "Meeting Rooms IIGiven an array of meeting time intervals intervals where intervals[i] = [starti, endi], return the minimum number of conference roo..."
---
...
class Solution {
    static bool cmp(vector& a,vector& b){
      //当开始和结束时间相同，要保证先下车
      int x = a[0] - b[0];
      return (x == 0 ? a[1] >& intervals) {
        vector > meetings;
        for(const vector& in: intervals){
        ...
        int cnt = 0;
        int maxv = 0;
        for(const vector& meet:meetings){
            cnt+=meet[1];
            maxv = max(maxv,cnt);
        }
        return maxv;
...
public:
    int minMeetingRooms(vector>& intervals) {
        ...
        for (const pair& meeting : meetings) {
            cnt += meeting.second;
