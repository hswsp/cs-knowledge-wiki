...
显然，为了可以在 ![image](https://g.yuque.com/gr/latex?O(1))的复杂度内取得当前中位数，我们应当令 `l` 为大根堆，`r` 为小根堆，并人为固定`l`和 `r`之前存在如下的大小关系：
...
对顶堆解法两个操作中耗时操作复杂度为 ![image](https://g.yuque.com/gr/latex?O(%5Clog%7Bn%7D))，![image](https://g.yuque.com/gr/latex?%5Clog)操作常数不会超过 3，在极限数据 ![image](https://g.yuque.com/gr/latex?10%5E7)情况下计算量仍然低于耗时操作复杂度为 ![image](https://g.yuque.com/gr/latex?O(C))（C 固定为 101）桶计数解法。
