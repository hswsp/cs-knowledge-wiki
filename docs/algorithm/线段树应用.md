...
线段树 $\textit{segmentTree}$是一个二叉树，**每个结点保存数组 **$\textit{nums}$**在区间**![image](https://images.spumn.eu.cc/blog/b588e11fbdbbb616.svg)** 的最小值、最大值或者总和等信息**（这里就保存求和信息）。线段树可以用树也可以用数组（堆式存储）来实现。对于数组实现，假设根结点的下标为 0，如果一个结点在数组的下标为 $\textit{node}$，那么它的左子结点下标为 $\textit{node} \times 2 + 1$，右子结点下标为 $\textit{node} \times 2 + 2$。

## 建树build函数


我们在结点 $\textit{node}$保存数组 $\textit{nums}$在区间![image](https://images.spumn.eu.cc/blog/b588e11fbdbbb616.svg)的总和。
...
度为2的节点有2个分支, 度为1结点有1个分支, 度为0的节点有0个分支。设分支数个数为![image](https://images.spumn.eu.cc/blog/cf15c9a8141aadb9.svg)，

![image](https://images.spumn.eu.cc/blog/56058dd10a9a5edf.svg)

另外![image](https://images.spumn.eu.cc/blog/853e71e688e725cd.svg) (**每个结点上面对应一个分支，除了根节点上面没有分支**)
...
结合公式 1和2 有![image](https://images.spumn.eu.cc/blog/e002dbf282af3f16.svg)
...
那么如果以`left = 2*n + 1`,`right = 2*n + 2`这种完全二叉树去计算下标的话，实际上的所需数组长度要比![image](https://images.spumn.eu.cc/blog/e2f7be293a11971c.svg)要大！！！

最极端的情况为：

在倒数第二层，第一个节点分裂出两个叶子节点，最后一个节点分裂出俩叶子节点，中间全部都是单个的叶子节点。那么此时把这个倒数第二层的叶子节点（总共不超过![image](https://images.spumn.eu.cc/blog/b9b01cc8a9c06a59.svg)个）占用最后一层两个虚拟位置的情况下，补全成完全二叉树就应该有不超过![image](https://images.spumn.eu.cc/blog/29fb459a0b04db36.svg)个叶子节点！！！！

****

**所以，总节点数不超过**![image](https://images.spumn.eu.cc/blog/f5ce54193afb88a2.svg)**个。所以数组总数取**`**4*nums.size()**`
