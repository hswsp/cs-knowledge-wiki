...
# 复杂度分析
## 时间复杂度
在整个代码实现过程中，我们仅仅出现了一层的 for 循环，没有出现任何 for 循环的嵌套，所以计数排序的时间复杂度为![image](https://images.spumn.eu.cc/blog/44b596d49a8a3bce.svg) 量级。

## 空间复杂度
由于计数排序过程中，我们使用到了一个 `**max - min + 1**` 大小的 **count[]** 数组，所以计数排序的空间复杂度为![image](https://images.spumn.eu.cc/blog/44b596d49a8a3bce.svg) 量级。

# 优缺点分析
1. 如果输入数据的范围  `**range = max - min + 1**` 不明显大于要待排序数组的长度 `**n = arr.length**`，则计数排序是相当高效的，比时间复杂度为![image](https://images.spumn.eu.cc/blog/a9fd649a671e6127.svg) 的快速和归并排序都优秀。
2. 计数排序不是基于比较的排序算法，时间复杂度为 ![image](https://images.spumn.eu.cc/blog/44b596d49a8a3bce.svg)，空间复杂度与数据范围成正比。
3. 计数排序通常用作另一个排序算法（例如基数排序）的子过程。
4. 计数排序可以使用部分哈希（partial Hashing）在![image](https://images.spumn.eu.cc/blog/c46388a0ff1512b0.svg)的时间内统计数据的频率。
5. 计数排序适用于负输入。
6. 计数排序不适用于小数的情况。
