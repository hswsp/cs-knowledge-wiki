...
设 $\textit{dist}[x][y]$表示从节点 x 到节点 y 的距离，假设树中距离最长的两个节点为 (x,y)，它们之间的距离为 $\textit{maxdist} = \textit{dist}[x][y]$，则可以推出**以任意节点构成的树最小高度一定为**$minheight=\lceil \frac{maxdist}{2}\rceil$**，且最小高度的树根节点一定在 节点 x 到节点 y 的路径上**。
