...
即$s=s_1+s_2$。**记**$\hat{s}$**为 s 的反序，那么最终得到的字符串**$s'+s$**可以写成**$\hat{s}_2 + s_1 + s_2$**的形式。**
...
考虑到 $s_1$是一个回文串，因此 $s_1 = \hat{s}_1$同样是$\hat{s}$ 的后缀。而$\hat{s}$ 可以写成$\hat{s}_2+\hat{s}_1$ 的形式，故而$\hat{s}=\hat{s}_2+s_1$。从而这道题其实就是**寻找最长字符串**$s_1$**，使得**$s_1$**是即是字符串s的前缀，也是字符串**$\hat{s}$**的后缀。**
