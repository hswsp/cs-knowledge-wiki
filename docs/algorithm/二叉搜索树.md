---
title: "二叉搜索树"
description: "首先，BST 的特性大家应该都很熟悉了：对于 BST 的每一个节点node，左子树节点的值都比node的值要小，右子树节点的值都比node的值大。对于 BST 的每一个节点node，它的左侧子树和右侧子树都是 BST。二叉搜索树并不算复杂，但我觉得它构建起了数据结构领域的半壁江山，直接基于 B..."
---
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdHDhO70O5T6qmmt2L4r8oGMWE3m6gib0via0nCcv0ficdKrGjqm6XIzQ70NCabkwZtblR7kyp7EQxkDg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdHDhO70O5T6qmmt2L4r8oGMxqOy6Djmkjice1mXhiaQ9tmtWHGTBg64ohcvHoGU3lTVlCJicnvufusmg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdHDhO70O5T6qmmt2L4r8oGM1B3zc4tCQLU5jKDQSyEib5NpC9bIiaib4k7I3nCiaDhGKecibib7Sw03oT2w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdHDhO70O5T6qmmt2L4r8oGMeoAeGRrjVuIdyKnCkS5MRibFTSIEl554dfmg1eQVWR2EumpHkW9uI5Q/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
第一道题是力扣第 96 题「不��的二叉搜索树」，给你输入一个正整数`n`，请你计算，存储`{1,2,3...,n}`这些值共有有多少种不同的 BST 结构。
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdFsfcAAK8DGTzPM7psSLiaqq5mWcpcaOepia3soypDcdhtIdmCf0YRVKzW2iaV4QayHYtibbAwFksqR5g/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdFsfcAAK8DGTzPM7psSLiaqqicDN8E0YqcgjGcc6wIGKJkV0mUh6wRh2nDMdCTe4Qkoic6xnIjeknCqA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
List build(int lo, int hi) {
    ...
    // 1、穷举 root 节点的所有可能。
    for (int i = lo; i  leftTree = build(lo, i - 1);
        List rightTree = build(i + 1, hi);
        // 3、给 root 节点穷举所有左右子树的组合。
 ...
 */
class Solution {
    void serialize(TreeNode* root, vector& sorted){
        if(!root) return;
...
public:
    vector> closestNodes(TreeNode* root, vector& queries) {
        ...
        for(auto& num:queries){
            vector::iterator maxi = lower_bound(sorted.begin(),sorted.end(),num);
            int index = maxi - sorted.begin();
            if(maxi==sorted.end()){
                res.push_back(vector{sorted[sorted.size()-1],-1});
            }else if(*maxi==num){
                res.push_back(vector{num,num});
            }else if(index==0){
                res.push_back(vector{-1,sorted[index]});
            }else{
                res.push_back(vector{sorted[index - 1],sorted[index]});
            }
        }
        return res;
    }
...
class Solution {
public:
    vector find(TreeNode* root, int target) {
    ...
    vector> closestNodes(TreeNode* root, vector& queries) {
        ...
        int n = queries.size();
        vector> ans(n, vector(2, -1));
        
        for (int i = 0; i 

**复杂度**
- 时间复杂度: 1813718137，如果树平衡则为 1813718137
- 空间复杂度: 1813718137
# 最后总结
