---
title: "股票买卖问题"
description: "这篇文章用「状态机」的技巧给出最优解，可以全部提交通过。不要觉得这个名词高大上，文学词汇而已，实际上就是 DP table，等会儿一讲就明白了。先随便抽一道题出来，看看别人发的解法：int maxProfit(vector&amp; prices) {     if(prices.empty..."
---
...
![image](https://mmbiz.qpic.cn/mmbiz_png/map09icNxZ4nPicwNq5syrSwnBc02yxG3aTWM5qlnc1ic8rc2v4ibuquTDUWOu7GVYqFAib4iaFxekON2AqKAPnAzicNw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
而这里，我们不用递归思想进行穷举，而是**利用「状态」进行穷举**。
...
![image](https://mmbiz.qpic.cn/mmbiz_png/map09icNxZ4nPicwNq5syrSwnBc02yxG3auom48UuPXWBQV7IRsa59yHe1TRQicDquqyRFOoia62BqHzofboyiay5IA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/map09icNxZ4kbOQM3dXVKdgSXOCibeUUK2nACKnCmYcLSOsGpvKc46tF7vzVSfIUISR0gd93cMPE9phGOZiaImlWw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

直接套状态转移方��，根据 base case，可以做一些化简：
...
int n = prices.length;
int[][] dp = new int[n][2];
for (int i = 0; i 显然 i = 0 时 `dp[i-1]` 是不合法的。这是因为我们没有对 i 的 base case 进行处理。那就简单粗暴地处理一下：
...
![image](https://mmbiz.qpic.cn/mmbiz_png/map09icNxZ4kbOQM3dXVKdgSXOCibeUUK2uibf92DLjHdyzfek9b7lfuia0mM4icmKgZB8ibJzkuxDY2e3zpzqeyUZtQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629904347086-d090de10-8e4d-4642-a774-93dff945b03d.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629904288533-4b1da6df-6b8d-4b78-8a09-4f8edb5607d0.png)
​
...
直接翻译成代码即可，注意状态转移方程改变后 base case 也要做出对应改变：：
...
for (int i = 0; i 为什么错误？我这不是照着状态转移方程写的吗？

还记得前面总结的「穷举框架」吗？就在强调必须穷举所有状态。其实我们之前的解法，都在穷举所有状态，只是之前的题目中 k 都被化简掉了，所以没有对 k 的穷举。比如说第一题，k = 1：

![image](https://mmbiz.qpic.cn/mmbiz_png/map09icNxZ4nPicwNq5syrSwnBc02yxG3aAfehqTCribLX230FrJWmuZxTSAicpI7j8KWN7BMTmg7FVgdBf4tNibhaQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
## 第六题，k = any integer 
**—— **Best Time to Buy and Sell Stock III/IV (Hard)

第六题和第五题类似，就是限定了你的最大交易次数，只要解决第六题就行了，看题目：

![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629903592305-ad2fcda3-cc97-4d40-88db-e3e3af5e9c62.png)

这题和 k = 2 没啥区别，可以直接套上一题的第一个解法。但是提交之后会出现一个超内存的错误，原来是传入的 k 值可以任意大，导致 dp 数组太大了。现在想想，交易次数 k 最多能有多大呢？

一次交易由买入和卖出构成，至少需要两天。**所以说有效的限制次数 k 应该不超过 n/2，如果超过，就没有约束作用了，相当于 k = +infinity。**这种情况是之前解决过的。ifne
...
int maxProfit(int k, vector& prices) {
    ...
    int dp_i0 = 0;
    int dp_i1 = INT_MIN;
    for(int i = 0;i至此，6 道题目通过一个状态转移方程全部解决。
