---
title: "NSum问题"
description: "双指针应用+递归本文就由浅入深，层层推进，用一个函数来解决所有 nSum 类型的问题。一、twoSum 问题力扣上的 twoSum 问题，题目要求返回的是索引，这里我来编一道 twoSum 题目，不要返回索引，返回元素的值：如果假设输入一个数组 nums 和一个目标和 target，请你返回 ..."
---
...
```cpp
vector> twoSumTarget(vector& nums, int target);
```
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629043279211-6fe93a78-c16d-4683-97fa-ade5ebc2c874.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629043296416-bee17ef1-98d9-45cd-9ede-b3f6da1a0ab4.png)
...
vector> twoSumTarget(
    ...
    vector> res;
    while (lo > threeSumTarget(vector& nums, int target) {
    // 数组得排个序
    sort(nums.begin(), nums.end());
    int n = nums.size();
    vector> res;
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629043328106-8bf863fb-30b5-4837-9900-12e8d6094bc3.png)
...
vector> fourSum(vector& nums, int target) {
    ...
    for (int i = 0; i > 
        ...
        // fourSum 的第一个数不能重复
        while (i > 
    threeSumTarget(vector& nums, int start, int target) {
        int n = nums.size();
        vector> res;
