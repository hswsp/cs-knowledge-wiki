---
title: "递归与栈"
description: "今天给大家讲解一道非常有趣的算法面试题，以非递归的形式来写快速排序。其实这也可以衍生出更多同类问题，非递归二叉树的前序、中序、后序遍历等等，这些问题的背后的思想是一致的，那就是用栈来手动模拟递归调用。道理很简单有没有，一句话就能说清楚，但问题是你真的理解了吗？该怎样用栈来手动模拟递归调用呢？你..."
---
...
![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPzOnze19pNHIJapCPj6mr5gAJq20qJ0kl3LGf5O55Dg4FAIT7xzwIzg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

然后将数组中小于该基准的数字放在左边、将大于该数字的放在右边：

![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPta3BwajREribA39icgG4XXbZJRzLxPN5u5icecibhznmeCSGKfiaL77WMVw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
```java
void quick_sort(vector&arr, int b, int e) {
    if (b >= e) return;
    int i = b - 1;
    for (int k = b; k 
...
```java
int partition(vector&arr, int b, int e, bool* sorted) {
  if (b > e || b == e) return -1;
...
![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPJF9GQ9ME5V2gyuRRHBew2GRyqKyBfiaKegT3l1bP19WpRDJIRACWBFw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

此时我们要对数组下标2到9的元素进行排序，把末尾的base作为基准进行划分：

![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPyLXRT5IIZEmmYAblyianLgWRYvMTyHEO8Upic1S2qdwQ8Myk8TX4g1sg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

假设划分后base放到了下标为5的位置，这样我们得到了两个子问题(2,3)以及(4,9)：

![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPq1OPmpe4TmiaCmxGsFY6za1sEwOOIbz1wB1BdBd6sCcswnic0pxiaOeZQ/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

由于经过base的划分后我们判断出该数组不是有序的(partition函数中sorted参数的作用)，因此我们需要将两个子问题(2,3)以及(4,9)放到栈中：

![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPX89aiaktWFNHnsOo6SEOvkZYTw9wxiah6PqEMXVfD4Qjlxc2nPuNcM8Q/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPbFRPiaDTp3e6kEM7rYKyiby56P46aMpMVeQ7jA2kEgS3fmDQ9mswP48Q/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPlM2HibJWJBIQJeVln4fJlnOFXGgXZ2M6L3slbZA63sUPwaeASfqGcJQ/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

看到非递归版本与递归版本的不同了吧：

![image](https://mmbiz.qpic.cn/mmbiz_png/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPPuXCVz4pNib2lXxDnmllz1vQ9jyQnXICEzrkF0TbnDfbPxFU6hF1McQ/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/mmbiz_jpg/9BK2WL6kfcTuItlyvJ1xbQ6mQYY8blaPMS7TnQOiay3tic4PtAibxbDSI8ZdA8xcazt54xcbCXGsLelVJZSxASGZA/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1)