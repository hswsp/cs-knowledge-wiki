---
title: "深度优先遍历"
description: "假设B+数的阶为N，一个简单的深度优先遍历程序如下。使用栈进行存储路径，关键在于倒序将子节点压栈："
---
...
请注意，这只是一个简单的示例，实际B+树可能包含更多的信息和功能。在实际应用中，您可能需要更复杂的B+树实现，包括插入、删除等操作。此示例主要用于演示使用栈进行深度优先搜索的基本思想。
...
// 定义B+树节点结构
struct BPlusTreeNode {
    vector<pair<int, int>> keys; // 键-磁盘位置对
    vector<BPlusTreeNode*> children;
    bool isLeaf;
...
int binarySearch(const vector<pair<int, int>>& keys, int target) {
    int left = 0;
    ...
    while (left <= right) {
        ...
        if (keys[mid].first == target) {
            return mid; // 找到了目标值
        } else if (keys[mid].first < target) {
            left = mid + 1;
        } else {
            right = mid - 1;
        }
...
BPlusTreeNode* depthFirstSearch(BPlusTreeNode* root, int target) {
    ...
    while (!nodeStack.empty()) {
        ...
        if (!currentNode->isLeaf) {
            ...
            // 叶子节点，查找到目标键时返回当前节点
            if (index < currentNode->keys.size() && currentNode->keys[index].first == target) {
                return currentNode;
            }
        }
    }
...
int main() {
    ...
    // 输出查找结果
    if (leafNode) {
        cout << "Key " << targetKey << " found in leaf node. Disk location: " << leafNode->keys[0].second << endl;
    } else {
        cout << "Key " << targetKey << " not found in B+ tree." << endl;
    }
...
在这个版本中，`std::lower_bound`通过自定义的比较函数来执行查找，类似于我们之前手动实现的二分查找。
...
// 深度优先搜索函数，加入二分查找和范围判断，后序遍历
void depthFirstSearchRangePostOrder(BPlusTreeNode* root, int a, int b) {
    if (!root) {
        return;
    }

    stack<pair<BPlusTreeNode*, bool>> nodeStack;  // 使用 pair 记录节点是否已访问
    ...
    while (!nodeStack.empty()) {
        ...
        if (visited) {
            ...
            auto upperBoundIt = upper_bound(currentNode->keys.begin(), currentNode->keys.end(), make_pair(b, 0),
                                            [](const pair<int, int>& p1, const pair<int, int>& p2) {
                                                return p1.first < p2.first;
                                            });

            int lowerIndex = lowerBoundIt - currentNode->keys.begin();
            int upperIndex = upperBoundIt - currentNode->keys.begin();

            // 处理在区间 [a, b] 之间的键值
            for (int i = lowerIndex; i < upperIndex; ++i) {
                cout << "Key: " << currentNode->keys[i].first << ", Disk location: " << currentNode->keys[i].second << endl;
            }
        } else {
            // 未访问，重新入栈，先访问子节点
...
int main() {
    ...
    cout << "Keys in the range [" << rangeStart << ", " << rangeEnd << "]: " << endl;
    depthFirstSearchRangePostOrder(root, rangeStart, rangeEnd);
...
在这个版本中，使用了一个额外的 `bool` 记录节点是否已经访问。当节点已经访问过时，表示当前节点的子节点已经被处理完毕，可以处理当前节点。这样实现的深度优先后序遍历。
...
void depthFirstSearchRange(BPlusTreeNode* root, int a, int b) {
    ...
    while (!nodeStack.empty()) {
        ...
        // 检查当前节点的范围是否与目标区间有交集
        if (currentNode->keys.back().first < a || currentNode->keys.front().first > b) {
...
int main() {
    ...
    cout << "Keys in the range [" << rangeStart << ", " << rangeEnd << "]: " << endl;
    depthFirstSearchRange(root, rangeStart, rangeEnd);
...
这里`std::upper_bound` 函数返回一个迭代器，指向在有序序列中大于某个值的第一个元素。如果找不到大于该值的元素，则返回指向序列末尾的迭代器。
...
void breadthFirstSearch(BPlusTreeNode* root) {
    ...
    while (!nodeQueue.empty()) {
        ...
        // 将当前节点的子节点入队
        for (BPlusTreeNode* child : currentNode->children) {
            if (child) {
...
这个版本中，使用队列将当前节点的子节点按照从左到右的顺序加入队列，实现了广度优先遍历。
...
BPlusTreeNode* breadthFirstSearch(BPlusTreeNode* root, int target) {
    ...
    while (!nodeQueue.empty()) {
        ...
        if (!currentNode->isLeaf) {
            // 非叶子节点，将相应的子节点入队
            for (int i = 0; i <= index && i < currentNode->children.size(); ++i) {
...
在这个版本中，广度优先搜索函数 `breadthFirstSearch` 使用 `std::lower_bound` 进行键的查找。类似于深度优先搜索，我们在非叶子节点时将相应的子节点入队，确保队列中的节点按照广度优先的顺序被处理。
