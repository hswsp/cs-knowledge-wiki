---
title: "常用的位运算技巧"
description: "本文分两部分，第一部分列举几个有趣的位操作，第二部分讲讲算法中常用的 n &amp; (n - 1) 操作以及异或运算，顺便把用到这些技巧的算法题列出来讲解一下。因为位操作很简单，所以假设读者已经了解与、或、异或这三种基本操作。位操作（Bit Manipulation）可以玩出很多奇技淫巧，但是这些技..."
---
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdFibOOvru90NjInwNskrznDWZq8Koh22aEicmAuicETsaDBA3OGQwqcKyOFNLKyICd2bv209B2uFbxibQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
`x&-x`表示取`x`最低位为1的那个数。即`m = x&-x`,则`x%m = 0`且1813718137, 如果x为奇数， m =1。例如： 
