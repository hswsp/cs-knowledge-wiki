...
求 $\frac{a}{c}$，可以把 $a = 2b$，$b = 3c$ 依次代入，得到 $\cfrac{a}{c} = \cfrac{2b} {c} = \cfrac{2 \cdot 3c} {c} = 6.0$；



求 $\frac{b}{a}$，很显然根据 $a = 2b$，知道 $\frac{b}{a}=0.5$，也可以把b 和 a 都转换成为 c 的倍数，$\frac{b}{a}=\frac{b}{2b}=\frac{3c}{6c}=\frac{1}{2}=0.5$；
...
例如已知 $\frac{a}{b}=3.0$， $\frac{d}{c}=4.0$  ，又已知$\frac{a}{d}=6.0$，现在合并结点 a 和 d 所在的集合，其实就是把 a 的根结点 b 指向 d 的根结 c，那么如何计算 b 指向 c 的这条有向边的权重呢？
