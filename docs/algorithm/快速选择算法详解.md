---
title: "快速选择算法详解"
description: "快速选择算法是一个非常经典的算法，和快速排序算法是亲兄弟。原始题目很简单，给你输入一个无序的数组nums和一个正整数k，让你计算nums中第k大的元素。那你肯定说，给nums数组排个序，然后取第k个元素，也就是nums[k-1]，不就行了吗？当然可以，但是排序时间复杂度是O(NlogN)，其中..."
---
...
总结一下，**快速选择算法就是快速排序的简化版**，复用了partition函数，快速定位第 k 大的元素。相当于对数组部分排序而不需要完全排序，从而提高算法效率，将平均时间复杂度降到1813718137。