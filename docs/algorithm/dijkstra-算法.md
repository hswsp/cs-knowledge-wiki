---
title: "图的抽象"
description: "其实，很多算法的底层原理异常简单，无非就是一步一步延伸，变得看起来好像特别复杂，特别牛逼。"
---
...
public:
    Dijkstra(int size, vector<vector<int> > times){
        this->size = size + 1;
        // 节点编号是从 1 开始的，所以要一个大小为 n + 1 的邻接表
        graph = new list<vector<int> >[size+1];
        for(int i = 1;i<=size;++i){
            graph[i] = list<vector<int> >();
        }
        for(auto &edge : times){
            int from = edge[0];
            int to = edge[1];
    ...
    vector<int> dijkstra(int start){
        ...
        priority_queue<State*, deque<State*>, decltype(cmp)> q(cmp);
