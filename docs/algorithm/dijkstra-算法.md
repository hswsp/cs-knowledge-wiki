---
title: "Dijkstra 算法"
description: "其实，很多算法的底层原理异常简单，无非就是一步一步延伸，变得看起来好像特别复杂，特别牛逼。但如果你看过历史文章，应该可以对算法形成自己的理解，就会发现很多算法都是换汤不换药，毫无新意，非常枯燥。比如，我们说二叉树非常重要，你把这个结构掌握了，就会发现 动态规划，分治算法，回溯（DFS）算法，B..."
---
...
比如，我们说二叉树非常重要，你把这个结构掌握了，就会发现 [动态规划](https://www.yuque.com/docs/share/9e4cf5e2-2cd2-4bbc-9f9c-0a8f191594a8?# 《动态规划详解》)，[分治算法](https://www.yuque.com/docs/share/d294626b-3c6a-4b91-85b4-419561f2bc38?# 《分治算法详解》)，[回溯（DFS）算法](https://www.yuque.com/docs/share/53b91676-36f8-4d84-b229-9d6aa39b6e17?# 《回溯算法详解》)，[BFS 算法框架](https://www.yuque.com/docs/share/a3af058f-0787-479f-a9a6-b56ced468f5b?# 《BFS 算法框架详解》)，[Union-Find 并查集算法](https://www.yuque.com/docs/share/cb9b34e5-3832-4e8d-be38-8507af0ed362?# 《并查集》)，[二叉堆实现优先级队列](https://www.yuque.com/docs/share/800f0502-72bf-4a5d-8207-740f7e6cc118?# 《二叉堆实现优先级队列》) 就是把二叉树翻来覆去的运用。

那么本文又要告诉你，Dijkstra 算法无非就是一个 BFS 算法的加强版，它们都是从二叉树的层序遍历衍生出来的。
...
前文 [图论第一期：遍历基础](https://www.yuque.com/docs/share/953f9e0e-33ae-4e43-a589-f228bcec4b67?# 《图论基础》) 说过「图」这种数据结构的基本实现，图中的节点一般就抽象成一个数字（索引），图的具体实现一般是「邻接矩阵」或者「邻接表」。

![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgIy9yKCeUblbYBViaDicibuFwU1HFjz9pKHfYCCk0iaMje5CHFWcQRVGVzQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

比如上图这幅图用邻接表和邻接矩阵的存储方式如下：

![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsg0nScpGC8pzxvIwRUJqEWxYczwlc0GuV8wOCw2zRyFTjVuMshhxGBCQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgQMNtxF34YCQsbbOHcuXprVr3WN4NojM8d43crlggwmMFksq6C4ibvfg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
如果对 BFS 算法不熟悉，可以看前文 [BFS 算法框架](https://www.yuque.com/docs/share/a3af058f-0787-479f-a9a6-b56ced468f5b?# 《BFS 算法框架详解》)，这里只是为了让你做个对比，所谓 BFS 算法，就是把算法问题抽象成一幅「无权图」，然后继续玩二叉树层级遍历那一套罢了。
...
但是，到了「加权图」的场景，事情就没有这么��单了，因为你不能默认每条边的「权重」都是 1 了，这个权重可以是任意正数（Dijkstra 算法要求不能存在负权重边），比如下图的例子：

![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgd5DV6sESzMJPAu8mL8wxMuOzdySZqku6vqt5n2NBP7med88WlsKpQQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

如果沿用 BFS 算法中的`step`变量记录「步数」，显然红色路径一步就可以走到终点，但是这一步的权重很大；正确的最小权重路径应该是绿色的路径，虽然需要走很多步，但是路径权重依然很小。

其实 Dijkstra 和 BFS 算法差不多，不过在讲解 Dijkstra 算法框架之前，我们首先需要对之前的**BFS的算法框架**进行如下改造：
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgkjn8sGlD4nKzl7KqzETcYbB2d1OEzak3KEOf7nS14EvMrKkxuR8MDw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
int[] dijkstra(int start, List[] graph) {
    ...
    // 定义：distTo[i] 的值就是节点 start 到达节点 i 的最短路径权重全局变量
    ...
    Queue pq = new PriorityQueue<>((a, b) -> {
...
public:
    Dijkstra(int size, vector > times){
        this->size = size + 1;
        ...
        for(auto &edge : times){
            ...
            // from -> List
            // 邻接表存储图结构，同时存储权重信息
            vector info {to, weight};
            graph[from].push_back(info);
        }

    }

    vector dijkstra(int start){
        // 图中节点的个数
        int V = this->size;
        cout distTo(V,INT_MAX);
        // base case，start 到 start 的最短距离就是 0
        distTo[start] = 0;
        ...
        while(!q.empty()){
            ...
            for (int nextNodeID : adj(curNodeID)) {
                ...
                if (distTo[nextNodeID] > distToNextNode) {
                    ...
                    distTo[nextNodeID] = distToNextNode;
                    // 将这个节点以及距离放入队列
                    q.push(new State(nextNodeID, distToNextNode));
                }
            }
        }
        return distTo;
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgSggM0BExqOUAeIyubSRic7EsGYgWnJJkjkuaYjsicswFjmlCwcYkiah7g/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

你下一步想遍历那个节点？就当前的情况来看，你觉得哪条路径更有「潜力」成为最短路径中的一部分？

从目前的情况来看，显然橙色路径的可能性更大嘛，所以我们希望节点`2`排在队列靠前的位置，优先被拿出来向后遍历。

所以我们使用`PriorityQueue`作为队列，让`distFromStart`的值较小的节点排在前面，这就类似我们之前讲 [贪心算法](https://www.yuque.com/docs/share/a957afc3-e053-4979-8b76-bdfa4ee5b00d?# 《跳跃游戏》) 说到的贪心思路，可以很大程度上优化算法的效率。
...
int dijkstra(int start, int end, List[] graph) {
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgXVaWUJ2DicjiasnnCblwofv4I0YA0qPBl3vbicGxtAJ9a0Lwjr8cQX3WA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgHceziciakX8hiarjUpUYz4UibSpJOTzg41MypFiascmINB1Pyq6UF8DBs9g/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
这样一想，是不是就在让你以左上角坐标为起点，以右下角坐标为终点，计算起点到终点的最短路径？Dijkstra 算法是不是���以做到？
...
// 返回坐标 (x, y) 的上下左右相邻坐标
List adj(int[][] matrix, int x, int y) {
...
int minimumEffortPath(int[][] heights) {
    ...
    // dp table 初始化为正无穷
    for (int i = 0; i  pq = new PriorityQueue<>((a, b) -> {
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdGiaE70bfibhZwtP90zPlWicsgh8XmZmKePSAJDrkdmYNqM7puZyJ6NmRPlEPG4nzwToBA931TJL16tg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
首先关于有向图和无向图，前文 [图算法基础](https://www.yuque.com/docs/share/953f9e0e-33ae-4e43-a589-f228bcec4b67?# 《图论基础》) 说过，无向图本质上可以认为是「双向图」，从而转化成有向图。
...
只不过，这道题的解法要**把优先级队列的排序顺序反过来**，一些 if 大小判断也要反过来，我们直接看解法代码吧：
