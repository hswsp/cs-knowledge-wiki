---
title: "原地更新技巧"
description: "双缓冲技术当数据量非常大时，画图可能须要几秒钟甚至更长的时间，并且有时还会出现闪屏现象，为了解决这些问题，采用双缓冲技术来画图。双缓冲即在内存中创建一个与屏幕画图区域一致的对象，先将图形绘制到内存中的这个对象上，再一次性将这个对象上的图形复制到屏幕上，这样能大大加快画图的速度。双缓冲实现步骤例..."
---
...
public:
    void gameOfLife(vector>& board) {
        ...
        for(int i = 0; i = 0 && nx = 0 && ny >= 1; //右移一位，用第二bit覆盖第一个bit。
            }
        }
    }
};
...
public:
    void gameOfLife(vector>& board) {
        ...
        // 遍历面板每一个格子里的细胞
        for (int row = 0; row = 0) && (c = 0) && (abs(board[r][c]) == 1)) {
                                liveNeighbors += 1;
                            }
                        }
                    }
                ...
                // 规则 1 或规则 3 
                if ((board[row][col] == 1) && (liveNeighbors  3)) {
                    // -1 代表这个细胞过去是活的现在死了
                    board[row][col] = -1;
                }
                // 规则 4
