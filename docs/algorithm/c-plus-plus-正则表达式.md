---
title: "C++ 正则表达式"
description: "正则程序库（regex）「正则表达式」就是一套表示规则的式子，专门用来处理各种复杂的操作。std::regex是C用来表示「正则表达式」（regular expression）的库，于C11加入，它是class std::basic_regex&lt;&gt;针对char类型的一个特化，还有一个针对wch..."
---
...
```cpp
std::regex reg(".*");
bool ret = std::regex_match("value", reg);
assert(ret);

ret = std::regex_match("value", reg);
assert(!ret);

std::regex reg1(".*");
ret = std::regex_match("value", reg1);
assert(ret);

ret = std::regex_match("value", std::regex("value"));
assert(ret);

// 使用basic文法
std::regex reg2(".*", std::regex_constants::basic);
ret = std::regex_match("value", reg2);
assert(ret);
```
...
```cpp
std::regex reg1(R"(.*)");
auto ret = std::regex_match("value", reg1);
assert(ret);
```
...
```cpp
std::cmatch m;
auto ret = std::regex_match("value", m, std::regex("(.*)"));
if (ret)
{
	std::cout 
...
![image](https://cdn.nlark.com/yuque/0/2021/jpeg/22382307/1638849061902-13ca1565-005b-4ed8-9730-d80b36c361a6.jpeg)
...
![image](https://cdn.nlark.com/yuque/0/2021/jpeg/22382307/1640404528327-539f2a0c-55a2-4eea-afa1-a47018e9c763.jpeg)

**非贪婪：**如果您只需要匹配开始和结束 h1 标签，下面的非贪婪表达式只匹配 
  
![image](https://cdn.nlark.com/yuque/0/2021/jpeg/22382307/1640404528301-fddbf6bd-c830-46b0-886a-84a1ce38dc4a.jpeg)
...
std::regex reg("(.*)");
std::cmatch m;
auto ret = std::regex_search("123value456", m, reg);
if (ret)
{
	for (auto& elem : m)
		std::cout 
...
对于「搜索」，在匹配结果中可以分别通过prefix���suffix来获取前缀和后缀，前缀即是匹配内容前面的内容，后缀则是匹配内容后面的内容。

那么若有多组符合条件的内容又如何得到其全部信息呢？这里依旧通过一个小例子来看：

```cpp
std::regex reg("(.*)");
std::string content("123value456centerhahahawindowthe end");
std::smatch m;
