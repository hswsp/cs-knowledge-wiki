---
title: "一、算法框架"
description: "前面所说的回溯算法就是DFS 算法，接下来，我们将介绍BFS算法框架。"
---
...
![](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629078956422-f6cea0a1-9c83-477c-8b44-72d4fac65852.png)
...
![](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629078974211-3ba2f4d9-f561-4de3-b955-69254eea7f5e.png)
...
![](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629079006252-fd25c6d3-09dc-439a-853e-837f77b71d54.png)
...
struct pair_hash
{ 
  size_t operator()(const pair<int,int>& r1) const
  {
    return hash<int>()(r1.first) ^ hash<int>()(r1.second);
  }
};
//equal相当于重载operator==
struct pair_equal
{ 
  size_t operator()(const pair<int,int>& r1,const pair<int,int>& r2) const
  {
    return r1.first==r2.first&&r1.second==r2.second;
  }
};
typedef unordered_set<pair<int,int>,pair_hash,pair_equal>  uset;
typedef std::function<bool(pair<int,int>,pair<int,int>)> Functional;

pair_equal equals;
Functional obj = equals;
vector<pair<int,int> > hdir={
    make_pair(-1,0),
    make_pair(1,0),
    make_pair(0,-1),
    make_pair(0,1)
}; 
vector<pair<int,int> > fdir = {
    make_pair(-1,0),
    make_pair(1,0),
    make_pair(0,-1),
    make_pair(0,1),
...
int n,m;
int maze(vector<vector<char>>& map,pair<int,int> start,pair<int,int> end,pair<int,int> fire){
    uset q1;
    uset visited;
    uset dead;//火场边界
    uset fvisited;
    ...
    while(!q1.empty()){
        ...
        for(auto cur:q1){
            ...
            visited.emplace(cur);
            for(auto x:hdir){
               pair<int,int> next = make_pair(cur.first+x.first, cur.second+x.second);
        ...
        for(auto f:dead){
            fvisited.emplace(f);
            for(auto x:fdir){
                pair<int,int> next = make_pair(f.first+x.first, f.second+x.second);
                if(next.first<0||next.first>=n||next.second<0||next.second>=m) continue;
...
![](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629079034879-d26484f7-a378-409f-bed8-531681a81ac9.png)



![](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629079048474-e68d28d0-816a-418b-b925-37cfee7c118f.png)
...
今年的世界冰球锦标赛在捷克举行。Bobek 已经抵达布拉格，他不是任何团队的粉丝，也没有时间观念。他只是单纯的想去看几场比赛。如果他有足够的钱，他会去看所有的比赛。不幸的是，他的财产十分有限，他决定把所有财产都用来买门票。 给出 Bobek 的预算和每场比赛的票价，试求：如果总票价不超过预算，他有多少种观赛方案。如果存在以其中一种方案观看某场比赛而另一种方案不观看，则认为这两种方案不同。 输入第一行，两个正整数 _N_ 和 _M_ (![image](https://cdn.nlark.com/yuque/__latex/a14e9b42caf1bb10b869246c30a9f644.svg))，表示比赛的个数和 Bobek 那家徒四壁的财产。 输入第二行，_N_ 个以空格分隔的正整数，均不超过，代表每场比赛门票的价格。 输出一行，表示方案的个数。由于 _N_ 十分大，注意：答案![image](https://cdn.nlark.com/yuque/__latex/3707205d16d13df41ad8e32ddcf82c13.svg)
