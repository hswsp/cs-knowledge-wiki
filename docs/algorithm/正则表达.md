...
![](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629080546408-38b8a21a-05ee-450d-8ec4-d8ac0e0dcc27.png)
...
1.  如果 B 的最后一个字符是正常字符，那就是看 `A[n-1]` 是否等于 `B[m-1]`，相等则看 ![image](https://g.yuque.com/gr/latex?A_%7B0..n-2%7D) 与 ![image](https://g.yuque.com/gr/latex?B_%7B0..m-2%7D)，不等则是不能匹配，这就是子问题。 
2.  如果 B的最后一个字符是`.`，它能匹配任意字符，直接看 ![image](https://g.yuque.com/gr/latex?A_%7B0..n-2%7D)与 ![image](https://g.yuque.com/gr/latex?B_%7B0..m-2%7D) 
3.  如果 B的最后一个字符是`*`它代表 `B[m-2]=c` 可以重复0次或多次，它们是一个整体 `c*`:  
情况一：`A[n-1]` 是 0 个 c，B 最后两个字符废了，能否匹配取决于 ![image](https://g.yuque.com/gr/latex?A_%7B0..n-1%7D) 和$ B_{0..m-3}$是否匹配（只能选不看`c*`这一种情况）  
情况二：![image](https://g.yuque.com/gr/latex?A%5Bn-1%5D)是多个 ![image](https://g.yuque.com/gr/latex?c) 中的最后一个（这种情况必须 ![image](https://g.yuque.com/gr/latex?A%5Bn-1%5D%3Dc) 或者 `c='.'`），所以 `**A**`** 匹配**完往前挪一个，`B` 继续匹配，因为可以匹配多个，继续看 ![image](https://g.yuque.com/gr/latex?A_%7B0..n-2%7D)和 ![image](https://g.yuque.com/gr/latex?B_%7B0..m-1%7D)是否匹配。（这里可以选择不看，或者看两种情况） 
...
注意递归和状态转移方程式相反的方向。状态转移是通过![image](https://g.yuque.com/gr/latex?f%5Bi-1%5D%5Bj-1%5D)推导![image](https://cdn.nlark.com/yuque/__latex/b39f967879b7469d5fc7c472584a79da.svg)。而递归框架是考虑![image](https://cdn.nlark.com/yuque/__latex/f02c17f8c82cefb5ce08ff88f0c276b9.svg)的时候带入![image](https://cdn.nlark.com/yuque/__latex/cb221188f6576918383f2359ba9a1223.svg)的情况。
...
![image](https://cdn.nlark.com/yuque/__latex/b39f967879b7469d5fc7c472584a79da.svg)**代表 A 的前 **![image](https://g.yuque.com/gr/latex?i)** 个和 B 的前 **![image](https://g.yuque.com/gr/latex?j)** 个能否匹配**



+ 对于前面两个情况，可以合并成一种情况 ![image](https://g.yuque.com/gr/latex?f%5Bi%5D%5Bj%5D%20%3D%20f%5Bi-1%5D%5Bj-1%5D)
+ 对于第三种情况，对于 `c*` 分为看和不看两种情况 
    - 不看：直接砍掉正则串的后面两个，$ f[i][j] = f[i][j-2]$
    - 看：正则串不动，主串前移一个，![image](https://g.yuque.com/gr/latex?f%5Bi%5D%5Bj%5D%20%3D%20f%5Bi-1%5D%5Bj%5D)
...
+ 空串和空正则是**匹配**的，![image](https://g.yuque.com/gr/latex?f%5B0%5D%5B0%5D%20%3D%20true)
+ 非空串和空正则必不匹配，![image](https://g.yuque.com/gr/latex?f%5B1%5D%5B0%5D%3D...%3Df%5Bn%5D%5B0%5D%3Dfalse)
+ 空串和非空正则，不能直接定义 `true`和 `false`，必须要计算出来。（比如$A= '' $ ,![image](https://g.yuque.com/gr/latex?B%3Da*b*c*)）
+ 非空串和非空正则，那肯定是需要计算的了。
...
我们开数组要开 ![image](https://g.yuque.com/gr/latex?n%2B1)，这样对于空串的处理十分方便。结果就是 ![image](https://g.yuque.com/gr/latex?f%5Bn%5D%5Bm%5D)
