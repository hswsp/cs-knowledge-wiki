---
title: "正则表达"
description: "给你一个字符串 s 和一个字符规律 p，请你来实现一个支持 . 和 * 的正则表达式匹配。. 匹配任意单个字符* 匹配零个或多个前面的那一个元素所谓匹配，是要涵盖 整个 字符串 s的，而不是部分字符串。示例 1：输入：s = aa p = a输出：false解释：a..."
---
...
前⽂多次强调过，**写递归的技巧是管好当下**，**之后的事抛给递归**。***具体到这⾥，不管 N 是多少，当前的选择只有两个：匹配 0 次、匹配1次***。所以可以这样处理：
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22382307/1629080546408-38b8a21a-05ee-450d-8ec4-d8ac0e0dcc27.png)

假设主串为 A，模式串为 B 从最后一步出发，需要关注最后进来的字符。假设 A 的长度为 n ，B 的长度为 m ，关注正则表达式 B 的最后一个字符是谁，它有三种可能，正常字符、`*` 和 `.`（点），那针对这三种情况讨论即可，如下：

-  如果 B 的最后一个字符是正常字符，那就是看 `A[n-1]` 是否等于 `B[m-1]`，相等则看 1813718137 与 1813718137，不等则是不能匹配，这就是子问题。 
-  如果 B的最后一个字符是`.`，它能匹配任意字符，直接看 1813718137与 1813718137 
 如果 B的最后一个字符是`*`它代表 `B[m-2]=c` 可以重复0次或多次，它们是一个整体 `c*`:
情况一：`A[n-1]` 是 0 个 c，B 最后两个字符废了，能否匹配取决于 1813718137 和$ B_{0..m-3}$是否匹配（只能选不看`c*`这一种情况）
情况二：1813718137是多个 1813718137 中的最后一个（这种情况必须 1813718137 或者 `c='.'`），所以 `**A**`** 匹配**完往前挪一个，`B` 继续匹配，因为可以匹配多个，继续看 1813718137和 1813718137是否匹配。（这里可以选择不看，或者看两种情况） ​
...
注意递归和状态转移方程式相��的方向。状态转移是通过1813718137推导1813718137。而递归框架是考虑1813718137的时候带入1813718137的情况。
...
1813718137**代表 A 的前 **1813718137** 个和 B 的前 **1813718137** 个能否匹配**

- 对于前面两个情况，可以合并成一种情况 1813718137
- 对于第三种情况，对于 `c*` 分为看和不看两种情况 
- 不看：直接砍掉正则串的后面两个，$ f[i][j] = f[i][j-2]$
- 看：正则串不动，主串前移一个，1813718137

## 初始条件

特判：需要考虑空串空正则
- 空串和空正则是**匹配**的，1813718137
- 非空串和空正则必不匹配，1813718137
- 空串和非空正则，不能直接定义 `true`和 `false`，必须要计算出来。（比如$A= '' $ ,1813718137）
- 非空串和非空正则，那肯定是需要计算的了。
...
我们开数组要开 1813718137，这样对于空串的处理十分方便。结果就是 1813718137
​
...
```cpp
typedef map,bool> pmap;
class Solution {
    pmap dict;// 备忘录 
    /*也可以使用vector > dict(ns,vector(np,-1));
    0  表示 false
    1  表示 true
    -1 表示没有遍历到*/
public:
    ...
    bool dp(string s, string p,int i, int j){
        ...
        pmap::iterator it = std::find_if(
            dict.begin(),dict.end(),[=](const pair,bool> && a){
                pair location = a.first;
...
bool isMatch(string s, string p) {
    ...
    for (int i = 0; i  0 && (s[i - 1] == p[j - 1] || p[j - 1] == '.')) {
                    f[i][j] = f[i - 1][j - 1];
                }
                continue;
            } 
