---
title: "丑数 II"
description: "「多路归并」的核心思想与合并 n 个有序链表极其相似，题目详情可见 [合并两个有序链表](https://leetcode.cn/problems/merge-two-sorted-lists/) [合并K个升序链表](https://le"
---
...
将 `n` 个有序链表的头节点加入小根堆的优先队列中，由于`n`个链表都是递增的顺序，所以第一个最小的元素肯定在这 `n` 个头节点中产生选择最小元素后，将该最小元素所在的链表后移一位，并将后一位元素加入队列中，以此类推...



![](https://pic.leetcode-cn.com/1655544615-lMqtMv-1.svg)
...
![](https://pic.leetcode-cn.com/1655544643-WVshQR-3.svg)
...
**注意**：需要去除相同元素。如上图第四小部分，第一条链表和第二条链表的当前值均为 `6`，如果选择把第一条链表中的 `6` 加入 `ans` 中，



那么第二条链条需要向前移动一格，否则 `6` 就加入了两次
...
public:
    ...
    int nthUglyNumber(int n) {
        ...
        for(int i=1;i<n;++i){
            ...
            //保持排序,注意有可能有num1==num2的情况，此时这俩都要+1！！！
            if(dp[i] == num1) p2++;
            if(dp[i] == num2)p3++;
            if(dp[i] == num3) p5++;
        }
...
public:
    int nthSuperUglyNumber(int n, vector<int>& primes) {
        ...
        vector<int> res; //结果集
        vector<int> pointer(size,0);//对应每一条分路的下标
        res.emplace_back(1); 
        for(int i=1;i<n;++i){
            int minnum = INT_MAX;
            for(int j = 0;j<size;++j){
                if(res[pointer[j]]< INT_MAX/primes[j])
                    minnum = min(minnum,res[pointer[j]]*primes[j]);
            }
            res.emplace_back(minnum);
            for(int j=0;j<size;++j){
                if(res[pointer[j]]< INT_MAX/primes[j] && res[pointer[j]]*primes[j]==minnum){
                    pointer[j]++;
                }
            }
        }
...
![](https://pic.leetcode-cn.com/1655544682-cuVtvU-4.svg)
...
+ 以 1 为分母，无符合要求组合 ❌
+ 以 2 为分母，有 ![image](https://g.yuque.com/gr/latex?%5Cfrac%7B1%7D%7B2%7D)
+ 以 3 为分布，有 ![image](https://g.yuque.com/gr/latex?%5Cfrac%7B1%7D%7B3%7D%20%5Crightarrow%20%5Cfrac%7B2%7D%7B3%7D)
+ 以 5 为分布，有 ![image](https://g.yuque.com/gr/latex?%5Cfrac%7B1%7D%7B5%7D%20%5Crightarrow%20%5Cfrac%7B2%7D%7B5%7D%20%5Crightarrow%20%5Cfrac%7B3%7D%7B5%7D)
...
![](https://pic.leetcode-cn.com/1655544710-aELrtm-5.svg)
...
对于 ![image](https://cdn.nlark.com/yuque/__latex/76f0ebb2acfd8ac383d28822b786b964.svg)我们只需要比较 ![image](https://g.yuque.com/gr/latex?ad%20%3C%20bc)即可
...
![](https://pic.leetcode-cn.com/1655544725-VwlcvW-6.svg)
...
**这个题目最佳的方法是「二分****➕****双指针」，详情可见 **[**二分搜索：第 K 小问题**](https://leetcode.cn/link/?target=https%3A%2F%2Flfool.github.io%2FLFool-Notes%2Falgorithm%2F%E4%BA%8C%E5%88%86%E6%90%9C%E7%B4%A2-%E7%AC%ACK%E5%B0%8F%E9%97%AE%E9%A2%98.html)
...
![](https://pic.leetcode-cn.com/1655544738-iKeYAs-9.svg)
...
**这个题目最佳的方法是「二分****➕****DFS」，详情可见 **[**二分搜索：第 K 小问题**](https://leetcode.cn/link/?target=https%3A%2F%2Flfool.github.io%2FLFool-Notes%2Falgorithm%2F%E4%BA%8C%E5%88%86%E6%90%9C%E7%B4%A2-%E7%AC%ACK%E5%B0%8F%E9%97%AE%E9%A2%98.html)
