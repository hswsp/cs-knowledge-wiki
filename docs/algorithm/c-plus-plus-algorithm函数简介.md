---
title: "C++ algorithm函数简介"
description: "统计函数统计元素出现的次数 头文件 algorithmstd::count(Iterator first, Iterator last, T &amp;val) 统计区间中某个元素出现的次数 std::count_if(InputIterator first, InputIterator last, ..."
---
...
int main()
{
    vector vec = {-7, 1, 10, 7, 2, 1};

    vector::iterator itMax = max_element(vec.begin(), vec.end());
    vector::iterator itMin = min_element(vec.begin(), vec.end());
...
```c
//判断 [first, last) 区域内的数据是否符合 std::less 排序规则，即是否为升序序列
