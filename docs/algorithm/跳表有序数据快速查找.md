...
![image](https://images.spumn.eu.cc/blog/112da0d3f88f140f.jpg)
...
![image](https://images.spumn.eu.cc/blog/3f00a35156b4810f.jpg)
...
![image](https://images.spumn.eu.cc/blog/890810fb387b7ade.jpg)
...
如果一个链表有 n 个结点，如果每两个结点抽取出一个结点建立索引的话，那么第一级索引的结点数大约就是 n/2，第二级索引的结点数大约为 n/4，以此类推第 m 级索引的节点数大约为 1813718137。

假如一共有 m 级索引，第 m 级的结点数为两个，通过上边我们找到的规律，那么得出1813718137，从而求得 1813718137。如果加上原始链表，那么整个跳表的高度就是1813718137。我们在查询跳表的时候，如果每一层都需要遍历 k  个结点，那么最终的时间复杂度就为 1813718137。
...
![image](https://images.spumn.eu.cc/blog/dde01cd3b1daa619.jpg)
...
跳表的空间复杂度分析并不难，如果一个链表有 n 个结点，如果每两个结点抽取出一个结点建立索引的话，那么第一级索引的结点数大约就是 n/2，第二级索引的结点数大约为 n/4，以此类推第 m 级索引的节点数大约为 1813718137，我们可以看出来这是一个等比数列。

这几级索引的结点总和就是 1813718137，所以跳表的空间复杂度为 o(n)。
...
![image](https://images.spumn.eu.cc/blog/bfe31117432b2a97.jpg)
...
`Random.nextDouble() `uses `Random.next() `twice to generate a double that has approximately uniformly distributed bits in its mantissa, so it is uniformly distributed in the range 0 to 1813718137.
​

`Random.nextInt(n) `uses `Random.next()` less than twice on average- it uses it once, and if the value obtained is above the highest multiple of n below `MAX_INT` it tries again, otherwise is returns the** value modulo n **(this prevents the values above the highest multiple of n below `MAX_INT` skewing the distribution), so returning a value which is uniformly distributed in the range 0 to n-1.
​

Prior to scaling by 6, the output of `Math.random() `is one of 1813718137possible values drawn from a uniform distribution.
​

Scaling by 6 doesn't alter the number of possible values, and casting to an int then forces these values into one of six 'buckets' (0, 1, 2, 3, 4, 5), each bucket corresponding to ranges encompassing either 1501199875790165 (1813718137)or 1501199875790166 of the possible values (as 6 is not a disvisor of 1813718137). This means that for a sufficient number of dice rolls (or a die with a sufficiently large number of sides), the die will show itself to be **biased towards the larger buckets.**
​
...
int cs() {
    int level = 1;
    for (int i = 1; random.nextInt(2) == 1 && i 这样才会更接近二分`（O(logn)）`，另外 `MAX_LEVEL=16 `大概能在 10W 数量级比较好的保持二分。
​
...
for (int i = level - 1; i >= 0; --i) {
    while (p.forwards[i] != null && p.forwards[i].data 让跳表变成了链表，想想这里50%的概率 `level=1` ，直接在最底层遍历。应当改成：

```java
int level = randomLevel();
......
for (int i = levelCount - 1; i >= 0; --i) {//levelCount为总层数
    while (p.forwards[i] != null && p.forwards[i].data ​