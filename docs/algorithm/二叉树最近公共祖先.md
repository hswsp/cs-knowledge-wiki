---
title: "二叉树最近公共祖先"
description: "Git 最常用的命令中有分支合并，主要就是merge和rebase两种方式。本文就用 Git 的rebase工作方式引出一个经典的算法问题：最近公共祖先（Lowest Common Ancestor，简称 LCA）。比如git pull这个命令，我们经常会用，它默认是使用merge方式将远端别..."
---

# 二叉树最近公共祖先

Git 最常用的命令中有分支合并，主要就是`merge`和`rebase`两种方式。本文就用 Git 的`rebase`工作方式引出一个经典的算法问题：**最近公共祖先**（Lowest Common Ancestor，简称 LCA）。
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHQ3jgWZ62Db0Se4zTCAz6JEut2FDEQISeg0DrujReXs24kk7ztk156HXHJtzrNV6ic2VPBTLsbwnQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHQ3jgWZ62Db0Se4zTCAz6JrY0ZvMzanlDUCNnQP762JKJnzJGtPYbsUsp8jbrwZaia7eTMnERLrDA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdHQ3jgWZ62Db0Se4zTCAz6Jic3UCtq3onR7PA6X2mibGeSZFqJ2KBwR9rOPob1X5r2FMcLibvictqEbxQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
