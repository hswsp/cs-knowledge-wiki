---
title: "代数结构速览：群、环、域"
description: "你每天都在跟有限域打交道，只是你可能不知道。"
---
...
打开一个 HTTPS 网页，AES-GCM 加密流量时在 ![image](https://cdn.nlark.com/yuque/__latex/71c1775d91d38ee726cc9b43b4cef34f.svg) 和 ![image](https://cdn.nlark.com/yuque/__latex/f016929649adf7a610515fcc62b07d84.svg) 上做乘法。手机扫一个二维码，Reed-Solomon 纠错码在 ![image](https://cdn.nlark.com/yuque/__latex/71c1775d91d38ee726cc9b43b4cef34f.svg) 上做多项式求值。NAS 上跑 ZFS，它的校验和基于 Reed-Solomon 编码。你用的每一张 CD、每一张 DVD、每一块 SSD 的 ECC，底层都是有限域运算。
...
本文从群、环、域的公理出发，逐步构造 GF(p) 和 ![image](https://cdn.nlark.com/yuque/__latex/1dc7421f19d4ef491cbbac0601d1131a.svg)，然后深入 AES、Reed-Solomon、GHASH、Shamir 秘密共享等具体应用，最后给出一个完整的 C 实现和工程实战经验。

![GF(2^8) 乘法流程](https://cdn.nlark.com/yuque/0/2026/svg/22382307/1776873443114-122d4b7d-81fd-4ecf-8be2-52c82f84d1ee.svg)
...
4. **逆元**：对任意 a 属于 G，存在 a<sup>(-1) 属于 G，使得 a * a</sup>(-1) = a^(-1) * a = e。
...
### 为什么 p 必须是素数
模 n 算术中，一个元素 a 有乘法逆元的充要条件是 gcd(a, n) = 1。如果 n 是素数，那么 {1, 2, …, n-1} 中的每个元素都与 n 互素，因此都有逆元。如果 n 不是素数，比如 n = 6，那么 2 * 3 = 0 (mod 6)，两个非零元素相乘得到零——这就是所谓的零因子，有零因子的结构不可能是域。
...
另一种方法是费马小定理：![image](https://cdn.nlark.com/yuque/__latex/c369a7262dfc25e40d19cc8e880b3149.svg)。这在 p 很大时可以用快速幂高效计算，但在 p 较小时不如扩展欧几里得直接。

### GF(p) 的乘法群是循环群
GF(p) 的非零元素在乘法下构成一个 p-1 阶的循环群。也就是说，存在一个**生成元**（原根）g，使得 {g^0, g^1, g^2, …, g^(p-2)} 恰好遍历 {1, 2, …, p-1}。
...
# GF(2^n)：扩展域的构造
当我们需要 256 个元素的有限域时，不能用 GF(256) = Z/256Z——因为 256 = 2^8 不是素数，Z/256Z 有零因子。正确的做法是构造 GF(2) 上的**多项式扩展域** GF(28)。
...
### 不可约多项式的角色
不可约多项式在 GF(2^n) 中的角色，类似于素数在 Z/pZ 中的角色。多项式环 GF(2)[x] 模一个不可约多项式 P(x)，得到的商环是一个域。如果 P(x) 可约，商环就会有零因子，不再是域。
...
加法的逆运算也是 XOR（因为在 GF(2) 中 -1 = 1），所以 GF(2^n) 中**加法和减法完全相同**。

### 乘法：多项式乘法模 P(x)
乘法是先做普通多项式乘法（系数在 GF(2) 中），然后对 P(x) 取模：
...
![image](https://cdn.nlark.com/yuque/__latex/c48bccea6a254b481e53607e20494f39.svg)
...
### GF(2^8) 乘法的 CLMUL 实现
用 CLMUL 做 GF(2^8) 乘法需要两步：无进位乘法得到最多 15 位的乘积，然后对 0x11B 取模：
...
### 原理
GF(2^8)* 是一个 255 阶的循环群，选定一个生成元 g（AES 中常用 g = 0x03），任意非零元素 a 都可以表示为 ![image](https://cdn.nlark.com/yuque/__latex/56de43d3127fe84e488312b6d15050bb.svg)。于是：

![image](https://cdn.nlark.com/yuque/__latex/0e943197934050acef18e9ab94bafee3.svg)
...
### 编码原理
设数据为 k 个字节 d_0, d_1, …, d_{k-1}，构造数据多项式：

![image](https://cdn.nlark.com/yuque/__latex/241f3d020aa89038abaddb75ed4d0dc9.svg)

其中 n = k + 2t，t 是要纠正的最大错误数。

Reed-Solomon 编码的核心是**生成多项式**：

![image](https://cdn.nlark.com/yuque/__latex/3beacb51aa3e821392818131866e6e9c.svg)

其中 alpha 是 GF(2^8) 的一个本原元素（通常取 alpha = 0x02）。

编码时，计算 D(x) 除以 G(x) 的余数 R(x)，码字为 C(x) = D(x) - R(x)。这保证了 C(x) 能被 G(x) 整除，即 C(alpha^i) = 0 对 i = 1, …, 2t 成立。
...
![image](https://cdn.nlark.com/yuque/__latex/65e499160ce00c5482ba5d713d0341b8.svg)
...
### Berlekamp-Massey 算法
BM 算法的核心思想是迭代地构造最短的线性反馈移位寄存器（LFSR），使其生成的序列与伴随式序列一致。算法维护两个多项式：当前最优的 Lambda(x) 和上一次更新的 B(x)。
...
![image](https://cdn.nlark.com/yuque/__latex/170176271f06bad9bf62f423adf939fb.svg)
...
![image](https://cdn.nlark.com/yuque/__latex/a89215b1c5f6e7077097b066720b64d7.svg)

其中 H 是认证密钥，乘法在 GF(2^128) 中进行。不可约多项式是：

![image](https://cdn.nlark.com/yuque/__latex/b3b2d377812729df910f976cdbb681f3.svg)
...
### 原理
1. 选择一个 k-1 次随机多项式 ![image](https://cdn.nlark.com/yuque/__latex/62a160ab9189ea0f435b8f75831e99f1.svg)，其中常数项是秘密 s。
...
| RS 生成多项式的根从 alpha^0 开始 | 与标准不兼容 | QR 码用 alpha<sup>0，CD 用 alpha</sup>1，看标准 |
...
**QR 码**：使用 GF(2^8) 上的 Reed-Solomon 编码，不可约多项式为 x^8 + x<sup>4 + x</sup>3 + x^2 + 1（0x11D，注意不是 AES 的 0x11B）。纠错等级从 L（7%）到 H（30%）不等。

**CD/DVD**：CD 使用交叉交织 Reed-Solomon 码（CIRC），两层 RS 编码分别纠正突发错误和随机错误。DVD 使用 RS 乘积码（RS-PC），将数据排成二维阵列，行列各做一次 RS 编码。

**ZFS 文件系统**：ZFS 的 RAID-Z2 和 RAID-Z3 使用 GF(2<sup>8) 上的 Reed-Solomon 编码来提供双盘和三盘容错。每个数据条带的校验计算都是在 GF(2</sup>8) 上做矩阵运算。
