...
打开一个 HTTPS 网页，AES-GCM 加密流量时在 ![image](https://images.spumn.eu.cc/blog/e3c591950946fc84.svg) 和 ![image](https://images.spumn.eu.cc/blog/1061fab876c19932.svg) 上做乘法。手机扫一个二维码，Reed-Solomon 纠错码在 ![image](https://images.spumn.eu.cc/blog/e3c591950946fc84.svg) 上做多项式求值。NAS 上跑 ZFS，它的校验和基于 Reed-Solomon 编码。你用的每一张 CD、每一张 DVD、每一块 SSD 的 ECC，底层都是有限域运算。
...
本文从群、环、域的公理出发，逐步构造 GF(p) 和 ![image](https://images.spumn.eu.cc/blog/cb1ada694c125958.svg)，然后深入 AES、Reed-Solomon、GHASH、Shamir 秘密共享等具体应用，最后给出一个完整的 C 实现和工程实战经验。
...
另一种方法是费马小定理：![image](https://images.spumn.eu.cc/blog/937a237a0687cc70.svg)。这在 p 很大时可以用快速幂高效计算，但在 p 较小时不如扩展欧几里得直接。
...
![image](https://images.spumn.eu.cc/blog/a89164b0317897bc.svg)
...
### 原理
GF(2^8)* 是一个 255 阶的循环群，选定一个生成元 g（AES 中常用 g = 0x03），任意非零元素 a 都可以表示为 ![image](https://images.spumn.eu.cc/blog/fed5c0ace6d6a63b.svg)。于是：

![image](https://images.spumn.eu.cc/blog/c61a4e719319d1e1.svg)
...
![image](https://images.spumn.eu.cc/blog/8fe53a4b94bf9b34.svg)

其中 n = k + 2t，t 是要纠正的最大错误数。

Reed-Solomon 编码的核心是**生成多项式**：

![image](https://images.spumn.eu.cc/blog/d92d91916973adf2.svg)
...
![image](https://images.spumn.eu.cc/blog/cd7bc22e1c372d70.svg)
...
![image](https://images.spumn.eu.cc/blog/c86700acc3b20fdf.svg)
...
![image](https://images.spumn.eu.cc/blog/aa2588dc055007ff.svg)

其中 H 是认证密钥，乘法在 GF(2^128) 中进行。不可约多项式是：

![image](https://images.spumn.eu.cc/blog/1967d75b06421169.svg)
...
### 原理
1. 选择一个 k-1 次随机多项式 ![image](https://images.spumn.eu.cc/blog/87528510a3684aea.svg)，其中常数项是秘密 s。
