...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/attention.png" alt=""></p>

<p align="center">图 3-1 缩放点积注意力机制</p>

注意力机制的工作过程主要包含两个步骤：

 - **计算注意力权重**：使用某种相似度函数度量每一个查询（query）向量和所有键（key）向量之间的关联程度。对于长度为 $m$ 的查询序列 Query 和长度为 $n$ 的键序列 Key，该步骤会生成一个尺寸为 $m \times n$ 的注意力分数矩阵。特别地，注意力机制使用点积作为相似度函数，这样相似的查询和键就会具有较大的点积。
...
由于点积可以产生任意大的数字，这会破坏训练过程的稳定性。因此注意力分数还需要乘以一个缩放因子来标准化它们的方差，然后用一个 softmax 函数标准化。这样就得到了最终的注意力权重 $w_{ij}$，表示第 $i$ 个查询向量与第 $j$ 个键向量之间的关联程度。
 ...
 - **更新词嵌入：**将权重 $w_{ij}$ 与对应的值（value）向量 $\boldsymbol{v}_1,…,\boldsymbol{v}_n$ 相乘以获得第 $i$ 个查询向量更新后的语义表示 $\boldsymbol{x}_i’ = \sum_{j} w_{ij}\boldsymbol{v}_j$。
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/multi_head_attention.png" alt=""></p>

<p align="center">图 3-2 多头注意力机制</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/transformer.jpeg" alt=""></p>

<p align="center">图 3-3 标准 Transformer 模型</p>

其中，编码器和解码器都各自包含有多层构件（building blocks）。图 3-4 展示了一个翻译任务的例子：

<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/encoder_decoder_architecture.png" alt=""></p>

<p align="center">图 3-4 Transformer 模型执行翻译任务</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/arrangements_of_layer_normalization.png" alt=""></p>

<p align="center">图 3-5 两种添加层归一化的方式</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/transformer_decoder.png" alt=""></p>

<p align="center">图 3-6 Transformer 解码器结构</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/local_attention_mask.png" alt=""></p>

<p align="center">图 3-7 局部注意力机制</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/sliding_window_attention.png" alt=""></p>

<p align="center">图 3-8 滑动窗口注意力机制</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/mha_mqa_gqa.png" alt=""></p>

<p align="center">图 3-9 多种注意力机制的对比</p>
...
<p align="center"><img src="https://images.spumn.eu.cc/assets/img/attention/mixture_of_experts.png" alt=""></p>

<p align="center">图 3-10 混合专家模型示意图</p>
