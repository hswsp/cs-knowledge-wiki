...
### Top-K选择
$\text{TopK}(g(x), k) = \{(i, g_i) \mid i \in \text{topk\_indices}(g(x), k)\}$
...
$\text{capacity} = \frac{\text{num\_tokens} \times k}{E} \times \text{capacity\_factor}$
...
## 7.5.1.3 辅助损失详解
### 总损失函数
$\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{LM}} + \lambda_1 \mathcal{L}_{\text{load}} +\lambda_2 \mathcal{L}_{\text{router\_zloss}}$

其中：

+ $\mathcal{L}_{\text{LM}}$：语言模型损失
+ $\mathcal{L}_{\text{load}}$：负载均衡损失
+ $\mathcal{L}_{\text{router\_zloss}}$：路由器z-loss（防止logits过大）

**Router Z-Loss**

$\mathcal{L}_{\text{router\_zloss}} = \frac{1}{B} \sum_{i=1}^{B} \left(\log \sum_{j=1}^{E}e^{z_{ij}}\right)^2$
...
$\text{experts\_per\_gpu} = \frac{E}{N}$
