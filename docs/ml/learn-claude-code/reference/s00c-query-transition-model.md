...
> 来源：[https://learn.shareai.run/zh/docs/s00c-query-transition-model/](https://learn.shareai.run/zh/docs/s00c-query-transition-model/)
...
-   `tool_result_continuation` 是正常主线继续
-   `max_tokens_recovery` 是输出被截断后的恢复继续
-   `compact_retry` 是上下文处理后的恢复继续
-   `transport_retry` 是基础设施抖动后的恢复继续
-   `stop_hook_continuation` 是外部控制逻辑阻止本轮结束
-   `budget_continuation` 是系统主动利用预算继续推进
