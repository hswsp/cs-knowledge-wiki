---
source: https://www.yuque.com/yangguangfanxing/zuyi8o/zq6pvbgpidr8ke6f
---

# Cipher Block Modes

> 原文链接：[https://www.yuque.com/yangguangfanxing/zuyi8o/zq6pvbgpidr8ke6f](https://www.yuque.com/yangguangfanxing/zuyi8o/zq6pvbgpidr8ke6f)
...
-   The **CBC** mode works in block of fixed size. Thus a [padding algorithm](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Padding) should be used to make the last block the same length after splitting the input data into blocks. Most applications use the [PKCS7 padding scheme](https://en.wikipedia.org/wiki/Padding_(cryptography)#PKCS#5_and_PKCS#7) or [ANSI X.923](https://en.wikipedia.org/wiki/Padding_(cryptography)#ANSI_X.923). In some scenarios the CBC block mode might be vulnerable to the ["padding oracle" attack](https://en.wikipedia.org/wiki/Padding_oracle_attack), so its is better to **avoid the CBC mode**.
...
![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692323263780-7d499249-8d1f-4841-a3d8-08d323b78392.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692323264288-7a8a68fd-0379-488a-8059-12023fbac222.png)

The **GCM** mode uses a **counter**, which is increased for each block and <u>calculated a</u> **<u>message</u>** **<u>authentication tag</u>** (MAC code) after each processed block. The final authentication tag is calculated from the last block. Like all counter modes, GCM works as a **stream cipher**, and so it is essential that a **different IV** is used at the start for each stream that is encrypted.
