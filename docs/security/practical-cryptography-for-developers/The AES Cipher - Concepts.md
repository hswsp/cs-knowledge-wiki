---
source: https://www.yuque.com/yangguangfanxing/zuyi8o/oxgrbrb0vvz6v04k
---

# The AES Cipher - Concepts

> 原文链接：[https://www.yuque.com/yangguangfanxing/zuyi8o/oxgrbrb0vvz6v04k](https://www.yuque.com/yangguangfanxing/zuyi8o/oxgrbrb0vvz6v04k)

The [Advanced Encryption Standard (AES)](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) cipher, also known as "**Rijndael**" is a popular, secure, widely used **symmetric key block cipher** algorithm, used officially as recommended encryption technology standard in the United States. **AES** operates using **block size of 128 bits** and symmetric **keys of length128**, **160**, **192**, **224** and **256** bits.
...
2.  The encryption **key** is derived from the encryption **password** using a key-derivation function (**KDF**), e.g. **Scrypt** (with certain parameters):![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604411619-08596571-c1b5-4a9f-8bbb-e4f71a670ff5.png)
3.  The **AES** encryption scheme takes as **input** the **input msg** + the encryption **key**. It produces as output the **ciphertext** + the randomly generated **IV** (128-bit salt) + the **MAC** code:![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604411699-7e430603-b3d0-4c77-8a34-253a4a1c3ed7.png)
...
-   If the encryption scheme is **not authenticated encryption** (e.g. AER-CTR), the MAC code is not calculated automatically by the AES encryption process and should be calculated additionally. The **MAC code** can be calculated from the **input msg**, using the encryption **key** (or some transformation of it) and some **MAC function** (like HMAC-SHA-256):![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604411620-00c0bad3-ac00-47d5-9cf6-79b501b60583.png)
-   The **ciphertext** is calculated through the **AES encryption algorithm**. It first **generates a random salt** (**IV**) and uses it to transform the **input msg** using the **encryption key**, through the AES cipher encryption logic:![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604411592-96fdeffc-4f36-4e4a-b6dc-0a06f219aacd.png)

1.  Finally, the encrypted output is generated. It holds the **ciphertext** + **IV** + **MAC**. Optionally, it holds also the algorithm settings.
...
2.  Use the decryption **key** and the **IV** from the encrypted message to decrypt the **ciphertext** using the **AES** algorithm decryption logic. The output is the **original message** (the **input msg**, which was previously passed through AES encryption):![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604411596-535c1f95-8030-4eb5-ab85-bcd63f411fc9.png)
...
1.  Calculate **HMAC** of the original message (obtained during the decryption):![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604412483-b5e77e4e-5c1c-4234-bd6a-a21e35872038.png)
2.  Compare the **encryption MAC** (the MAC of the input message, before the encryption) with the **decryption MAC** (the MAC of the original message, recovered by the decryption):![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1692604412515-b9dac537-240a-45be-b289-2eee5b309364.png)
