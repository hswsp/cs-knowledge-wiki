---
source: https://www.yuque.com/yangguangfanxing/zuyi8o/pt90tagyotrkb8q5
---

# Digital Signatures

> 原文链接：[https://www.yuque.com/yangguangfanxing/zuyi8o/pt90tagyotrkb8q5](https://www.yuque.com/yangguangfanxing/zuyi8o/pt90tagyotrkb8q5)
...
![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1693360783826-fb039c90-b562-4d12-baad-0731061e546a.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/22382307/1693360786037-11af3709-edac-4f6c-94f2-523ff7c9fe1e.png)
...
-   The [RSA sign](https://en.wikipedia.org/wiki/RSA_(cryptosystem)#Signing_messages) algorithm computes a message **hash**, then **encrypts** the hash with the private key exponent to obtain the **signature**. The obtained signature is an **integer** number (the RSA encrypted message hash).
...
The **random value k** (generated when the signature is computed) opens a potential vulnerability: i<u>f two</u> <u>different messages</u> <u>are signed using the same value of</u> **<u>k</u>** <u>and the same</u> **<u>private key</u>**<u>, then an attacker can compute the signer's private key directly</u> (see <https://github.com/tintinweb/ecdsa-private-key-recovery>).
...
The **random value k** (generated when the signature is computed) opens a potential vulnerability: <u>if two different messages are signed using the same value of</u> **<u>k</u>** <u>and the same</u> **<u>private key</u>**<u>, then an attacker can compute the signer's private key directly</u> (see <https://github.com/tintinweb/ecdsa-private-key-recovery>).
...
-   The **EdDSA signing** algorithm generates a deterministic (not random) integer **r** (computed by **hashing** the **message** and the hash of the **private key**), then computes the **signature** `{Rs, s}`, where **Rs** is computed from **r** and **<u>s</u>** <u>is computed from the</u> **<u>hash</u>** <u>of (the</u> **<u>message</u>** <u>+ the</u> **<u>public key</u>** <u>derived from the private + the number</u> **<u>r</u>**<u>)</u> + the **private key**. The signature is **deterministic** (the same message signed by the same key always gives the same signature).
...
|                                                                                 |                                                                             |                                                                                 |
|---------------------------------------------------------------------------------|-----------------------------------------------------------------------------|---------------------------------------------------------------------------------|
| **Text**                                                                        | **EdDSA-Ed25519**                                                           | **ECDSA-secp256k1**                                                             |
