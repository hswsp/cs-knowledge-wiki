---
title: "如何优化 TCP？"
description: "三次握手优化、TCP Fast Open、缓冲区与拥塞算法调优。"
---
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/a67fbc0dfb72-2.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/11e27d1f0bd8-3.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/dd59a91e7bc7-4.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/05b21fef8b17-5.jpg)
...
只有同步了序列号才有可靠传输，TCP 许多特性都依赖于序列号实现，比如流量控制、丢包重传等，这也是三次握手中的报文称为 SYN 的原因，SYN 的全称就叫 _Synchronize Sequence Numbers_（同步序列号）。



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/d7cf171c9db1-6.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/9c7a29fcbd5f-7.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/fa38d34d1a50-8.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/b5acafbdeece-9.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/934eb5b4b11c-10.jpg)
...
要想增大半连接队列，**不能只单纯增大 tcp_max_syn_backlog 的值，还需****一同增大 somaxconn 和 backlog****，也就是****增大 accept 队列****。否则，只单纯增大 tcp_max_syn_backlog 是无效的。**



增大 tcp_max_syn_backlog 和 somaxconn 的方法是修改 Linux 内核参数：

![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/472618b3d35f-11.jpg)



增大 backlog 的方式，每个 Web 服务都不同，比如 Nginx 增大 backlog 的方法如下：

![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/16a0225ea306-12.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/712cea056951-13.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/db51188736ee-14.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/04c3064366c1-15.jpg)
...
如果进程不能及时地调用 accept 函数，就会造成 accept 队列（也称全连接队列）溢出，最终导致**建立好的 TCP 连接被丢弃**。



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/7106abd9c4b5-16.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/179536738530-17.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/cc3813a5a7d8-18.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/9b5d7325c29b-19.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/cfe8d1d0e0bc-20.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/7ae24c859c85-21.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/f7f098b7c5d1-22.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/45bc6cfcfb17-TCP_20option_E5_AD_97_E6_AE_B5_20-_20TFO.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/7ef0a43129f4-23.jpg)
...
**TCP Fast Open 功能****需要客户端和服务端同时支持****，才有效果。**
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/71d046c89454-24.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/c0ed9ae5d823-25.jpg)
...
+ 当主动方收到这个 ACK 后，连接状态由 FIN_WAIT1 变为 FIN_WAIT2，也就是表示**主动方的****发送通道****就关闭了**。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/35cb9f4677d6-26.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/06ee3d04c336-27.jpg)



你可能会好奇，这 0 表示几次？**实际上当为 0 时，特指 8 次**，从下面的内核源码可知：

![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/f128f72b7063-28.jpg)
...
对于普遍正常情况时，调低 tcp_orphan_retries 就已经可以了。如果**遇到恶意攻击，FIN 报文根本无法发送出去**，这由 TCP 两个特性导致的：
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/753ea47d05da-29.jpg)
...
这时，**如果连接是用 ****shutdown 函数关闭****的，连接****可以一直处于**** FIN_WAIT2 状态，因为它可能还可以发送或接收数据。但对于 ****close 函数关闭****的孤儿连接，由于无法再发送和接收数据，所以这个状态不可以持续太久，而 ****tcp_fin_timeout 控制****了这个状态下连接的持续时长**，默认值是 60 秒：



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/f850fddb0551-30.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/0c5fef07a207-6385cc99500b01ba2ef288c27523c1e7.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/61b5873dcf5b-3a81c23ce57c27cf63fc2b77e34de0ab.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/eb2646c17590-TIME-WAIT_E8_BF_9E_E6_8E_A5_E6_AD_A3_E5_B8_B8_E5_85_B3_E9_97_AD.drawio.png)
...
这与孤儿连接 FIN_WAIT2 状态默认保留 60 秒的原理是一样的，**因为这两个状态都需要保持 2MSL 时****长。MSL 全称是 Maximum Segment Lifetime，它定义了一个报文在网络中的最长生存时间**（报文每经过一次路由器的转发，IP 头部的 TTL 字段就会减 1，减到 0 时报文就被丢弃，这就限制了报文的最长存活时间）。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/179f7182bbb7-33.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/5b15286230e3-34.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/187cbccb12b4-35.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/7f1d2650c6fa-TCP_20option_E5_AD_97_E6_AE_B5-_E6_97_B6_E9_97_B4_E6_88_B3.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/99a07a7784b3-36.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/573054b20227-37.jpg)
...
由于 TCP 是双全工的协议，所以是会出现两方同时关闭连接的现象，也就是**同时发送了 FIN 报文**。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/1fa7d3bb5762-38.jpg)



接下来，**双方在等待 ACK 报文的过程中，****都等来了 FIN 报文。这是一种新情况，所以连接会进入一种叫做 CLOSING 的新状****态，它替代了 FIN_WAIT2 状态**。接着，双方内核回复 ACK 确认对方发送通道的关闭后，进入 TIME_WAIT 状态，等待 2MSL 的时间后，连接自动关闭。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/fac144da02b5-39.jpg)
...
当 TIME_WAIT 状态过多时，还可以通过设置 `tcp_tw_reuse` 和 `tcp_timestamps` 为 1 ，将 TIME_WAIT 状态的端口复用于作为客户端的新连接，注意该参数**只适用于客户端**。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/1c37c89ed838-40.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/96e45d02e864-41.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/dbbcf3d48f22-42.jpg)
...
这个窗口大小最大值，在当今高速网络下，很明显是不够用的。所以后续有了扩充窗口的方法：**在 TCP 选项字段定义了****窗口扩大因子****，用于扩大 TCP 通告窗口，其值大小是 **`**2^14**`**，这样就使 TCP 的窗口大小从 16 位扩大为 30 位（**`**2^16 * 2^ 14 = 2^30**`**），所以此时窗口的最大值可以达到 1GB。**



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/035e7db0b4e0-TCP_20option_E5_AD_97_E6_AE_B5-_E7_AA_97_E5_8F_A3.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/b10a74dfc646-43.jpg)



要使用窗口扩大选项，通讯双方必须在**各自的 SYN 报文中发送**这个选项：
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/08d0adb00676-44.jpg)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/dccfeab97e01-45.jpg)
...
而接收缓冲区的调整就比较复杂一些，先来看看设置**接收缓冲区范围的 **`**tcp_rmem**`** 参数**：

![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/718887e7ed1b-46.jpg)
...
发送缓冲区的调节功能是自动开启的，**而****接收缓冲区则需要配置 **`**tcp_moderate_rcvbuf**`** 为 1 来开启调节功能**：



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/fb0935ae373a-47.jpg)
...
接收缓冲区调节时，怎么知道**当前内存是否紧张**或充分呢？这是通过 `tcp_mem` 配置完成的：

![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/576ff71f7db8-48.jpg)
...
在高并发服务器中，为了兼顾网速与大量的并发连接，**我们应当****保证缓冲区的动态调整的最大值达到带宽时延积****，而最小值保持默认的 4K 不变即可。而对于内存紧张的服务而言，调低默认值是提高并发的有效手段。**
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/tcp/9c86a0698ab7-49.jpg)
