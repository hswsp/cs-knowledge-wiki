...
description: "基于 ECDHE 的前向安全握手流程与抓包分析。"
---
...
我在上一篇已经介绍了[RSA 握手的过程](https://www.yuque.com/yangguangfanxing/rrzw64/evwb8o)，今天这一篇就「从理论再到实战抓包」介绍 **ECDHE 算法**。



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/06eec075924d-ecdhe_E6_8F_90_E7_BA_B2.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/262c1322c66d-_E6_8C_87_E6_95_B0_E4_B8_8E_E5_AF_B9_E6_95_B0.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/610a0baf0a38-_E6_B1_82_E5_AF_B9_E6_95_B0.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/c390185ceda1-_E7_A6_BB_E6_95_A3_E5_AF_B9_E6_95_B0.png)
...
**特别是当****模数 p 是一个很大的质数****，即使知道底数 a 和真数 b ，在现有的计算机的计算水平是几乎无法算出离散对数的，这就是 DH 算法的数学基础。**
...
现假设小红和小明约定使用 DH 算法来交换密钥，那么基于离散对数，小红和小明需要先确定**模数和底数**作为算法的参数，这两个参数是公开的，用 P 和 G 来代称。
...
A 和 B 也是**公开**的，因为根据离散对数的原理，从**真数**（A 和 B）反向计算对数 a 和 b 是非常困难的，至少在现有计算机的计算能力是无法破解的，如果量子计算机出来了，那就有可能被破解，当然如果量子计算机真的出来了，那么密钥协商算法就要做大的升级了。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/bedd7b4d0681-dh_E7_AE_97_E6_B3_95.png)
...
DHE 算法由于计算性能不佳，因为需要做大量的乘法，为了提升 DHE 算法的性能，所以就出现了现在广泛用于密钥交换算法 —— **ECDHE 算法(**Elliptic Cure Diffie-Hellman ephemeral**)**。
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/a9bde9f4d95f-ech_tls_E6_8F_A1_E6_89_8B.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/0f313fd3a3ff-ech_clinethello.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/5089ae195e12-ech_serverhello.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/33254db565fd-ech_certificate.png)



这一步就和 RSA 握手过程有很大到区别了，因为服务端选择了 ECDHE 密钥协商算法，所以会在发送完证书后，发送「**Server Key Exchange**」消息(即RSA中证书里的公钥，这里是椭圆曲线的公钥)。



![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/f6ab58929c18-ech_serverkey.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/f45f8c77b1cc-ech_serverhellodone.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/77442f98d3ed-ech_clientkeyexchange.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/f800b12d7c33-ech_schangecipherspec.png)
...
![](https://images.spumn.eu.cc/cs-knowledge-wiki/csapp/network/http/a22ee1df113c-ech_encryptedhandshakemessage.png)
