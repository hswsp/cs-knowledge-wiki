...
重载 operator[]，这⼀步返回是实值 value(即pair.second)的引⽤，假如原先没有定义 map 对象，即你访问的键值 key 不存在，则会⾃动新建⼀个 map 对象，键值 key 为你访问的键值key，实值 value 为空，看下⾯的例⼦就明⽩了。
