...
![img](https://images.spumn.eu.cc/blog/cbcbbdef99aa0439.webp)
...
![LSM-Tree 存储结构 1 ](https://images.spumn.eu.cc/blog/9a0c8479d2e0bd27.png)

![LSM-Tree 存储结构 2](https://images.spumn.eu.cc/blog/2f7d3e79ff6ef27f.png)
...
![LSM-Tree 写数据流程图](https://images.spumn.eu.cc/blog/f701df367bcdd8a3.png)
...
![img](https://images.spumn.eu.cc/blog/5275a9e5fdda8fab.jpg)
...
![ LSM-Tree SSTable’s merge](https://images.spumn.eu.cc/blog/98f92a37a466f78f.png)
...
![img](https://images.spumn.eu.cc/blog/93c593ba9beeedb5.jpg)
...
4. 把内存里面不可变的Memtable给dump到**硬盘上**的SSTable层中，此步骤也称为**Minor Compaction**，这里需要注意**在L0层的SSTable是没有进行合并的，所以这里的key range在多个SSTable中可能会出现重叠**，在层数大于0层之后的SSTable，不存在重叠key。
   ...
   ![The Base LSM Algorithm](https://images.spumn.eu.cc/blog/d2be8149b7496104.png)
...
5. 当每层的磁盘上的SSTable的体积超过一定的大小或者个数，也会周期的进行合并。此步骤也称为**Major Compaction**，这个阶段会真正 的**清除掉被标记删除掉的数据以及多版本数据的合并**，避免浪费空间，注意由于**SSTable都是有序的**，我们可以直接采用merge sort进行高效合并。
   ...
   ![Basic Compaction](https://images.spumn.eu.cc/blog/98f92a37a466f78f.png)
...
![通过软件实现的技巧像布隆过滤器或者硬件（大文件cache）来优化读性能](https://images.spumn.eu.cc/blog/048e93ddc6988f81.png)
...
最后有的同学可能会问道，为什么LSM不直接顺序写入磁盘，而是需要在内存中缓冲一下？ 这个问题其实很容易解答，**单条写的性能肯定没有批量写来的快**，这个原理其实在Kafka里面也是一样的，虽然kafka给我们的感觉是写入后就落地，但其实并不是，本身是 可以根据条数或者时间比如200ms刷入磁盘一次，这样能大大提升写入效率。此外在LSM中，在磁盘缓冲的另一个好处是，**针对新增的数据，可以直接查询返回**，能够避免一定的IO操作。
...
[OceanBase存储架构概述](https://www.oceanbase.com/docs/enterprise-oceanbase-database-cn-10000000000354478)
