...
在文章[kafka 和 pulsar 的架构设计](https://www.yuque.com/yangguangfanxing/td6rrt/fp1z3w)中介绍了 Pulsar 存算分离的架构，其中 broker 只负责计算，由 BookKeeper 负责底层的存储，我还画了这样一张图说明 BookKeeper 读写分离的设计：

![图片](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdG5IbhehuGHelIbXYZtTyRDLYPiaIFj7eVr75MpcHabiclJicbYqpJDIN2BkmuSDKhvxxPLia8dJCdJ6g/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1)
...
就我的理解，LSM 树其实不是一种数据结构，而是一种存储方案。这里面涉及三个重要的数据组件：`memtable`，`log`，`SSTable`（**Sorted String Table**），正如在[kafka 和 pulsar 的架构设计](https://www.yuque.com/yangguangfanxing/td6rrt/fp1z3w) 中画的这幅图：

![图片](https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdG5IbhehuGHelIbXYZtTyRDLYPiaIFj7eVr75MpcHabiclJicbYqpJDIN2BkmuSDKhvxxPLia8dJCdJ6g/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1)

其中`Journal`就是`log`，`Entry Log`就是若干`SSTable`的集合，叫法不同罢了。

`memtable`是红黑树或者跳表这样的有序**内存**数据结构，起到缓存和排序的作用，把新写入的数据按照键的大小进行排序。当`memtable`到达一定大小之后，会被转化成`SSTable`格式刷入磁盘持久化存储。

`SSTable`（Sorted String Table）说白了就是一个特殊格式的文件，其中的**数据按照键的大小排列**，你可以把它类比成一个有序数组。而 LSM 树，说白了就是若干`SSTable`的集合。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdG5IbhehuGHelIbXYZtTyRDV28CpEQeuXDNLFCONg0Cp24gYNTGYrrK6IUdr0j7XSPw71yXBNTmRA/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdG5IbhehuGHelIbXYZtTyRDk9vqObGkuWmFg43ZubjusVoL65dSNvAAaIe8PAdibWVoocOwRFojOKw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
这样，越靠上层的数据越新，越靠下层的数据越旧，且算法保证**同一层的若干 SSTable 的 key 不存在重叠**：

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/gibkIz0MVqdG5IbhehuGHelIbXYZtTyRDiaKibQNMo1qKfzeSYf0jRhRefR6Hb1J0Jia2dHJnJQe9E5a2Vkna8RR0Q/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

[https://github.com/facebook/rocksdb/wiki/Leveled-Compaction](https://github.com/facebook/rocksdb/wiki/Leveled-Compaction)

那么假设给一个目标键`key27`，我们只需要从上到下遍历层，并在每一层中使用 [二分查找算法](https://www.yuque.com/yangguangfanxing/coma88/igcu21) 找到键区间包含`key27`的`SSTable`，然后用布隆过滤器快速判断一下`key27`是否不存在这个`SSTable`中。如果可能存在，由于`SSTable`中的键也是有序的，可以再次运用[二分查找](https://www.yuque.com/yangguangfanxing/coma88/igcu21)在`SSTable`中找到键对应的值。

这样，借助 LSM 树的层级结构和`SSTable`的有序性，就能利用二分搜索提升查找效率，避免线性查找键值对。

以上就是本文的全部内容，LSM 树的设计思路比较易于理解，但实现起来还有不少细节，如果你对具体实现感兴趣，我可以推荐一些学习资料：

LevelDB 的代码仓库：[https://github.com/google/leveldb/issues](https://github.com/google/leveldb/issues)

RocksDB 的 wiki：[https://github.com/facebook/rocksdb/wiki](https://github.com/facebook/rocksdb/wiki)
