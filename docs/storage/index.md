# 存储 / Storage

> 数据库内核、分布式存储、存储引擎、CMU 15-445 课程笔记、存储协议与硬件介绍。

## 分布式存储

- [HDFS 简介](./distributed-storage/hdfs-intro)
- [OceanBase 架构图文档](./distributed-storage/OceanBase架构图文档.html)
- [SeekDB Architecture](./distributed-storage/seekdb-architecture.html)

## 存储引擎

- [LSM Tree：高性能海量数据写数据结构](./storage-engine/lsm-tree)
- [聊聊MySql事务,MVCC](./storage-engine/mvcc.md)

## 存储协议

- [NAS 各种协议](./protocols/nas-ge-zhong-xie-yi)
- [SATA 与 NVMe](./protocols/cun-chu-ji-shu-jie-shao-sata-yu-nvme)

## 硬件介绍

- [存储硬件技术名词概览](./hardware/技术名词概览)

## Intro to Database System

- [进入 Intro to Database System](./cmu15-445/)

## Vector Databases in Modern AI Applications

- [进入 Vector Databases in Modern AI Applications](./vector-database/)

## LSM-Tree in a Week

- [进入 LSM-Tree in a Week](./lsm-tree/)

> 分布式一致性（Paxos / Raft）请见 [系统设计 → 分布式共识](../system-design/#分布式共识)
