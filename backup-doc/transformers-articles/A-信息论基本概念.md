...
交叉熵(Cross Entropy)是信息论中另一个重要的概念，它在机器学习，特别是分类问题中有着广泛的应用。
...
其中 $p$ 是真实分布（ground truth），$q$ 是预测分布。交叉熵衡量的是使用分布 $q$ 来编码来自分布 $p$ 的样本所需的平均比特数。

### A.5.2 交叉熵与KL散度的关系

交叉熵与相对熵（KL散度）有着密切的关系。我们可以通过以下推导来理解：
...
由于真实分布 $p$ 的信息熵 $H(p)$ 是一个常数（对于给定的数据集），因此在优化过程中，最小化交叉熵等价于最小化KL散度。这解释了为什么在机器学习中，我们常常使用交叉熵作为损失函数，因为它实际上是在最小化预测分布与真实分布之间的KL散度。

### A.5.3 交叉熵在分类问题中的应用

在分类问题中，假设样本的真实标签为 $y$（one-hot编码），模型的预测概率为 $\hat{y}$，则交叉熵损失函数为：

$$
\mathcal{L} = -\sum_{i=1}^{C} y_i \log \hat{y}_i
$$

其中 $C$ 是类别数。对于二分类问题（$C=2$），交叉熵损失简化为：
...
交叉熵作为损失函数有以下优点：
1. 当预测概率与真实标签差异较大时，梯度较大，参数更新快
2. 当预测概率接近真实标签时，梯度较小，参数更新慢
3. 这种特性使得模型能够快速学习，同时避免在接近最优解时震荡

在深度学习框架中，交叉熵损失函数通常与Softmax激活函数结合使用，形成Softmax交叉熵损失，这是多分类问题中最常用的损失函数之一。
