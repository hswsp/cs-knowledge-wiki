...
> 来源：[https://learn.shareai.run/zh/s11/](https://learn.shareai.run/zh/s11/)
...
-   如果你开始分不清“为什么这一轮还在继续”，先回 [`s00c-query-transition-model.md`](./reference/s00c-query-transition-model)，重新确认 transition reason 为什么是独立状态。
-   如果你在恢复逻辑里又把上下文压缩和错误恢复混成一团，建议顺手回看 [`s06-context-compact.md`](./reference/s06-context-compact)，区分“为了缩上下文而压缩”和“因为失败而恢复”。
-   如果你准备继续往 `s12` 走，建议把 [`data-structures.md`](./reference/data-structures) 放在旁边，因为后面任务系统会在“恢复状态之外”再引入新的 durable work 状态。
