...
> 来源：[https://learn.shareai.run/zh/docs/data-structures/](https://learn.shareai.run/zh/docs/data-structures/)
...
-   先不懂词，就回 [`glossary.md`](./glossary)。
-   先不懂边界，就回 [`entity-map.md`](./entity-map)。
-   如果卡在 `TaskRecord` 和 `RuntimeTaskState`，继续看 [`s13a-runtime-task-model.md`](./s13a-runtime-task-model)。
-   如果卡在 MCP 为什么还有 resource / prompt / elicitation，继续看 [`s19a-mcp-capability-layers.md`](./s19a-mcp-capability-layers)。
...
-   `s10`
-   [`s10a-message-prompt-pipeline.md`](./s10a-message-prompt-pipeline)
...
这份总表最好配合 [`s00-architecture-overview.md`](./s00-architecture-overview) 和 [`entity-map.md`](./entity-map) 一起看。
