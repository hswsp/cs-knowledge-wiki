...
> 来源：[https://learn.shareai.run/zh/docs/team-task-lane-model/](https://learn.shareai.run/zh/docs/team-task-lane-model/)
...
1.  先看 [`s15-agent-teams.md`](./s15-agent-teams)，确认长期队友在讲什么。
2.  再看 [`s16-team-protocols.md`](./s16-team-protocols)，确认请求-响应协议在讲什么。
3.  再看 [`s17-autonomous-agents.md`](./s17-autonomous-agents)，确认自治认领在讲什么。
4.  最后看 [`s18-worktree-task-isolation.md`](./s18-worktree-task-isolation)，确认隔离执行车道在讲什么。

如果你开始混：

-   回 [`entity-map.md`](./entity-map) 看模块边界。
-   回 [`data-structures.md`](./data-structures) 看记录结构。
-   回 [`s13a-runtime-task-model.md`](./s13a-runtime-task-model) 看“目标任务”和“运行时执行槽位”的差别。
