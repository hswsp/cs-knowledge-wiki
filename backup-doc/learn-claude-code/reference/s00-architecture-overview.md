...
> 来源：[https://learn.shareai.run/zh/docs/s00-architecture-overview/](https://learn.shareai.run/zh/docs/s00-architecture-overview/)
...
如果你对这些词不熟，不要硬扛。  
应该立刻去看术语表：[`glossary.md`](./glossary)

如果你想先知道“这套仓库到底教什么、不教什么”，建议配合看：

-   [`teaching-scope.md`](./teaching-scope)
...
-   [`s00f-code-reading-order.md`](./s00f-code-reading-order)
...
| 文档                                                         | 用途                                                       |
|--------------------------------------------------------------|------------------------------------------------------------|
| [`teaching-scope.md`](./teaching-scope)                   | 帮你分清哪些内容属于教学主线，哪些只是维护者侧补充         |
| [`data-structures.md`](./data-structures)                 | 帮你集中理解整个系统的关键状态和数据结构                   |
| [`s00f-code-reading-order.md`](./s00f-code-reading-order) | 帮你把“章节顺序”和“本地代码阅读顺序”对齐，避免重新乱翻源码 |
...
| 文档                                                                   | 它补的是什么                                                                                  |
|------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| [`s00d-chapter-order-rationale.md`](./s00d-chapter-order-rationale) | 为什么这套课要按现在这个顺序讲，哪些重排会把读者心智讲乱                                      |
| [`s00e-reference-module-map.md`](./s00e-reference-module-map)       | 参考仓库里真正重要的模块簇，和当前课程章节是怎样一一对应的                                    |
| [`s00a-query-control-plane.md`](./s00a-query-control-plane)         | 为什么一个更完整的系统不能只靠 `messages[] + while True`                                      |
| [`s00b-one-request-lifecycle.md`](./s00b-one-request-lifecycle)     | 一条请求如何从用户输入一路流过 query、tools、permissions、tasks、teams、MCP 再回到主循环      |
| [`s02a-tool-control-plane.md`](./s02a-tool-control-plane)           | 为什么工具层不只是 `tool_name -> handler`                                                     |
| [`s10a-message-prompt-pipeline.md`](./s10a-message-prompt-pipeline) | 为什么 system prompt 不是模型完整输入的全部                                                   |
| [`s13a-runtime-task-model.md`](./s13a-runtime-task-model)           | 为什么任务板里的 task 和正在运行的 task 不是一回事                                            |
| [`s19a-mcp-capability-layers.md`](./s19a-mcp-capability-layers)     | 为什么 MCP 正文先讲 tools-first，但平台层还要再补一张地图                                     |
| [`entity-map.md`](./entity-map)                                     | 帮你把 message、task、runtime task、subagent、teammate、worktree、MCP server 这些实体彻底分开 |
