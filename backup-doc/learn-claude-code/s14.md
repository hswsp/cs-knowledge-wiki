...
> 来源：[https://learn.shareai.run/zh/s14/](https://learn.shareai.run/zh/s14/)
...
-   如果你还没完全分清 `schedule`、`task`、`runtime task` 各自表示什么，先回 [`s13a-runtime-task-model.md`](./reference/s13a-runtime-task-model)。
-   如果你想重新看清“一条触发最终是怎样回到主循环里的”，可以配合读 [`s00b-one-request-lifecycle.md`](./reference/s00b-one-request-lifecycle)。
-   如果你开始把“未来触发”误以为“又多了一套执行系统”，先回 [`data-structures.md`](./reference/data-structures)，确认调度记录和运行时记录不是同一个表。
